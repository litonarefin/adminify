...
class Adminify_Memory_Usage {
	...
	public function __construct() {
		add_action( 'wp_dashboard_setup', array( $this, 'jltwp_adminify_memory_usage_widget' ) );
	...
	public function jltwp_adminify_memory_usage_widget() {
		add_meta_box(
			...
			array( $this, 'jltwp_adminify_memory_usage_widget_details' ),
			'dashboard',
			'side',
			'high'
		);
	...
	public function jltwp_adminify_memory_usage_admin_css() {
		...
		if ( $screen->id == 'dashboard' ) {
			$my_site_space_css  = '';
			$my_site_space_css .= '';
			echo '<style>' . wp_strip_all_tags( $my_site_space_css ) . '</style>';
		}
	}
	...
	public function jltwp_adminify_memory_usage_widget_details() {
		...
		if ( strpos( WP_CONTENT_DIR, ABSPATH ) !== false ) {
			...
			if ( $entire_site_size > 0 ) {
				$topitems['Entire Site'] = $this->jltwp_adminify_memory_usage_dir_size_display( array( $entire_site_size ) );
			}
		} else {
			...
			if ( $entire_abs_size > 0 && $entire_content_size > 0 ) {
				$topitems['Entire Site'] = $this->jltwp_adminify_memory_usage_dir_size_display( array( $entire_abs_size, $entire_content_size ) );
			}
		}
		...
		<p><span class="spacedark">' . __( 'Memory Used by ', 'adminify' ) . '</span></p>';
		...
		foreach ( $contents as $name => $value ) {
			...
			if ( false === ( get_transient( $value ) ) ) {
				echo '<span class="spacedark">' . Utils::wp_kses_custom( $name ) . '</span>: ' . $this->jltwp_adminify_memory_usage_dir_size_display( array( $this->jltwp_adminify_memory_usage_dir_size( $value ) ) ) . '<br />';
			} else {
				echo '<span class="spacedark">' . Utils::wp_kses_custom( $name ) . '</span>: ' . size_format( get_transient( $value ), 2 ) . '<br />';
			}
		}
	...
	public function jltwp_adminify_memory_usage_dir_size_display( $sizes = array() ) {
		$found_error = '';
		$total_size  = 0;
