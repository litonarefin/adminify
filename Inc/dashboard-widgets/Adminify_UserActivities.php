...
class Adminify_UserActivities {
	...
	public function jltwp_adminify_set_date_time() {
		...
		$date = date_i18n( get_option( 'date_format' ), strtotime( date( 'Y-m-d', time() ) ) );
		$time = date_i18n( get_option( 'time_format' ), strtotime( date( 'H:i:s', time() ) ) );
                        ...
                        <td>
                            <div class="media">
                                <div class="wp-adminify-user-avatar media-left is-pulled-left image">
                                    ' . get_avatar( $current_user->user_email, 36, '', '', [ 'class' => 'is-rounded' ] ) . '
