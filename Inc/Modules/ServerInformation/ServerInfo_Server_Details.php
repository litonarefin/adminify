...
class ServerInfo_Server_Details
{
    ...
    public function init()
    {
        ...
        ?> </h1>
			...
			<p><?php 
        echo  __( 'Interesting information about your web server. You can also use <a href="http://linfo.sourceforge.net/" target="_blank" rel="noopener">linfo</a> or <a href="https://phpsysinfo.github.io/phpsysinfo/" target="_blank" rel="noopener">phpsysinfo</a> to get more information about the web server', 'adminify' ) ;
        ?>.</p>

			<p><?php 
        echo  __( 'In the most cases you can modify some server settings like "PHP Memory Limit" or "PHP Post Max Size" by upload and modify a <code>php.ini</code> file in the WordPress <code>/wp-admin/</code> folder. Learn more about <a href="https://www.wpbeginner.com/wp-tutorials/how-to-increase-the-maximum-file-upload-size-in-wordpress/" target="_blank" rel="noopener">here</a>', 'adminify' ) ;
        ?>.</p>
        ...
        echo  PHP_OS ;
        ?> / <?php 
        echo  PHP_INT_SIZE * 8 . __( 'Bit OS', 'adminify' ) . ' (' . php_uname() . ')' ;
        ?></td>
					</tr>
					<tr>
						<td><?php 
        ...
        ?>:</td>
						<td><?php 
        echo  Utils::wp_kses_custom( $_SERVER['SERVER_SOFTWARE'] ) ;
        ?></td>
					</tr>
					<tr>
						<td><?php 
        ...
        ?>:</td>
						<td><?php 
        echo  Utils::wp_kses_custom( $_SERVER['SERVER_ADDR'] ) ;
        ?></td>
					</tr>
					<tr>
						<td><?php 
        ...
        ?>:</td>
						<td><?php 
        echo  Utils::wp_kses_custom( $_SERVER['SERVER_PORT'] ) ;
        ?></td>
					</tr>
					<tr>
						<td><?php 
        ...
        ?>:</td>
						<td><?php 
        echo  date( 'Y-m-d H:i:s', time() ) . ' (' . current_time( 'mysql' ) . ')' ;
        ?></td>
					</tr>
					<tr>
						<td><?php 
        ...
        ?>:</td>
						<td><?php 
        echo  date_default_timezone_get() . ' (' . $server_info->get_wp_timezone() . ')' ;
        ?></td>
					</tr>
					<tr>
						<td><?php 
        ...
        if ( 'UTC' !== $default_timezone ) {
            echo  Utils::wp_kses_custom( $no ) . sprintf( __( 'Default timezone is %s - it should be UTC', 'adminify' ), $default_timezone ) . '</span>' ;
        } else {
        ...
        if ( function_exists( 'ini_get' ) ) {
            ...
            ?>:</td>
							<td><?php 
            echo  (int) ini_get( 'upload_max_filesize' ) . ' MB (' . size_format( wp_max_upload_size() ) . ')' ;
            ?></td>
						</tr>
						<tr>
							<td><?php 
            ...
            ?>:</td>
							<td><?php 
            echo  size_format( $server_info->convert_memory_size( ini_get( 'post_max_size' ) ) ) ;
            ?></td>
						</tr>
						<tr>
							<td><?php 
            ...
            ?>:</td>
							<td><?php 
            echo  ini_get( 'max_execution_time' ) . ' ' . esc_html__( 'Seconds', 'adminify' ) ;
            ?></td>
						</tr>
						<tr>
							<td><?php 
            ...
            if ( $gdl ) {
                echo  Utils::wp_kses_custom( $yes ) . ' / ' . esc_html__( 'Version', 'adminify' ) . ': ' . $gdl['GD Version'] ;
            } else {
                echo  Utils::wp_kses_custom( $no ) ;
            }
