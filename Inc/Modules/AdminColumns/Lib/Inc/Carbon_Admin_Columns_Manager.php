...
namespace WPAdminify\Inc\Modules\AdminColumns\Lib\Inc;
...
abstract class Carbon_Admin_Columns_Manager {
	...
	protected $sorted_columns = [];

	protected $columns_objects = [];

	static function modify_columns( $type, $object_types = null ) {
		 $type = str_replace( ' ', '_', ucwords( str_replace( '_', ' ', $type ) ) );
		$class = __NAMESPACE__ . '\Carbon_Admin_Columns_Manager_' . $type;
	...
	private function __construct( $object_types = [] ) {
	...
	public function modify_admin_columns( $columns ) {
		...
		foreach ( $this->sorted_columns as $column_name ) {
			...
			$columns = [ $column_name => $column_to_move ] + $columns;
		}
	...
	public function add( $columns ) {
		...
		foreach ( $columns as $column ) {
			if ( ! is_a( $column, __NAMESPACE__ . '\Carbon_Admin_Column' ) ) {
	...
	public function render_column_value( $looped_column_name, $object_id ) {
		echo Utils::wp_kses_custom( $this->get_column_value( $looped_column_name, $object_id ) );
	}
	...
	public function admin_head() {
		...
		if ( $css ) {
			...
			<style type="text/css">
				<?php echo Utils::wp_kses_custom( $css ); ?>
			</style>
