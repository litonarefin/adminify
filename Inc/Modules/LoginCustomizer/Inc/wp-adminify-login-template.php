...
if ( ! is_customize_preview() ) {
	$login_customizer_page = get_page_by_path( 'wp-adminify-login-customizer' );
...
function jltwp_adminify_login_header( $title = 'Log In', $message = '', $wp_error = null ) {
	...
	if ( $shake_error_codes && $wp_error->has_errors() && in_array( $wp_error->get_error_code(), $shake_error_codes, true ) ) {
		add_action( 'login_head', 'jltwp_adminify_shake_js', 12 );
	}
	...
	<body class="login wp-adminify <?php echo esc_attr( implode( ' ', $classes ) ); ?>">

		<div class="wp-adminify-general-actions">
			<div id="wp-adminify-templates-section" class="wp-adminify-preview-event" data-section="jltwp_adminify_customizer_template_section">
				<span class="dashicons dashicons-tagcloud"></span>
			</div>
			<div id="wp-adminify-layout-section" class="wp-adminify-preview-event" data-section="jltwp_adminify_customizer_layout_section">
				<span class="dashicons dashicons-layout"></span>
			</div>
			<div id="wp-adminify-background-section" class="wp-adminify-preview-event" data-section="jltwp_adminify_customizer_bg_section">
		 ...
		 * @since 4.6.0
		 */
		do_action( 'jltwp_adminify_login_header' );
...
} // End of jltwp_adminify_login_header()
		 ...
		 */
function jltwp_adminify_login_footer( $input_id = '' ) {
	...
	if ( ! $interim_login ) {
		?>
				...
				printf( _x( '&larr; <span id="wp-adminify-back-to-site">Back to</span> %s', 'site' ), get_bloginfo( 'title', 'display' ) );
				?>
					</a>
				</p>
			<?php
	...
	?>
		 ...
		 * @since 3.1.0
		 */
		do_action( 'jltwp_adminify_login_footer' );
	...
	<?php
		 ...
		 * @since 3.0.0
		 */
function jltwp_adminify_shake_js() {
	?>
	...
	<?php
		 ...
		 */
function jltwp_adminify_retrieve_password() {
	$errors = new WP_Error();
	...
	$action = 'resetpass';
		...
		switch ( $action ) {
			case 'confirm_admin_email':
				...
				jltwp_adminify_login_header( __( 'Confirm your administration email' ), '', $errors );
		...
		<form class="admin-email-confirm-form" name="admin-email-confirm-form" action="<?php echo esc_url( site_url( 'wp-login.php?action=confirm_admin_email', 'login_post' ) ); ?>" method="post">
			...
			case 'retrievepassword':
				if ( $http_post ) {
					$errors = jltwp_adminify_retrieve_password();
				...
				jltwp_adminify_login_header( __( 'Lost Password' ), '<p class="message">' . __( 'Please enter your username or email address. You will receive a link to create a new password via email.' ) . '</p>', $errors );
		...
		<p id="nav">
			<a href="<?php echo esc_url( wp_login_url() ); ?>"><?php _e( 'Log in' ); ?></a>
				...
				jltwp_adminify_login_footer( 'user_login' );
				break;
			...
			case 'rp':
				...
				if ( ( ! $errors->has_errors() ) && isset( $_POST['pass1'] ) && ! empty( $_POST['pass1'] ) ) {
					...
					setcookie( $rp_cookie, ' ', time() - YEAR_IN_SECONDS, $rp_path, COOKIE_DOMAIN, is_ssl(), true );
					jltwp_adminify_login_header( __( 'Password Reset' ), '<p class="message reset-pass">' . __( 'Your password has been reset.' ) . ' <a href="' . esc_url( wp_login_url() ) . '">' . __( 'Log in' ) . '</a></p>' );
					jltwp_adminify_login_footer();
					exit;
				}
				...
				jltwp_adminify_login_header( __( 'Reset Password' ), '<p class="message reset-pass">' . __( 'Enter your new password below.' ) . '</p>', $errors );
		...
		<p id="nav">
			<a href="<?php echo esc_url( wp_login_url() ); ?>"><?php _e( 'Log in' ); ?></a>
				...
				jltwp_adminify_login_footer( 'user_pass' );
				break;
			...
			case 'register':
				...
				jltwp_adminify_login_header( __( 'Registration Form' ), '<p class="message register">' . __( 'Register For This Site' ) . '</p>', $errors );
		...
		<p id="nav">
			...
			case 'confirmaction':
				...
				jltwp_adminify_login_header( __( 'User action confirmed.' ), $message );
				jltwp_adminify_login_footer();
				exit;
			...
			default:
				...
				jltwp_adminify_login_header( __( 'Log In' ), '', $errors );
		...
		<form name="loginform" class="show-only_login" id="loginform" action="<?php echo esc_url( site_url( 'wp-login.php', 'login_post' ) ); ?>" method="post">
			...
			<p class="forgetmenot"><input name="rememberme" type="checkbox" id="rememberme" value="forever" <?php checked( $rememberme ); ?> />
				<label for="rememberme">
					<span id="wp-adminify-remember-me">
			...
			<p id="nav">
					...
					if ( ! isset( $_GET['checkemail'] ) || ! in_array( $_GET['checkemail'], array( 'confirm', 'newpass' ), true ) ) {
						...
						?>
					<a id="wp-adminify-lost-password" href="<?php echo esc_url( wp_lostpassword_url() ); ?>"><?php _e( 'Lost your password?' ); ?></a>
						<?php
					}
