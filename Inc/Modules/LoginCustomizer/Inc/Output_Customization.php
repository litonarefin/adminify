...
if (!class_exists('Output_Customization')) {
    ...
    class Output_Customization
    {
        ...
        public function __construct()
        {
            ...
            add_action('wp_adminify_add_templates', [$this, 'add_templates'], 10, 1);
        ...
        {
            return ['template-01', 'template-02', 'template-03', 'template-04'];
        }
        ...
        public function add_templates($selected_template)
        {
            ...
            if (jltwp_adminify()->can_use_premium_code__premium_only()) {
                return include_once(WP_ADMINIFY_PATH . 'Pro/LoginCustomizerPro/templates/' . $selected_template . '.php');
            }
        ...
        */
        ...
        public function login_error_messages($error)
        {
            ...
            if (
                isset($errors) && apply_filters('wp_adminify_display_custom_errors', true)
            ) {
                ...
                if ($this->options['login_error_messages']) {
                    ...
                    // Username
                    if (in_array('error_incorrect_username',    $error_codes)) return $invalid_username;
                    if (in_array('error_empty_username',      $error_codes)) return $empty_username;
                    if (in_array('error_exists_username',     $error_codes)) return $username_exists;

                    // Password
                    if (in_array('error_incorrect_password',  $error_codes)) return $invalid_pasword;
                    if (in_array('error_empty_password',      $error_codes)) return $empty_password;

                    // Email
                    if (in_array('error_incorrect_email',       $error_codes)) return $invalid_email;
                    if (in_array('error_empty_email',         $error_codes)) return "</br>" . $empty_email;

                    // registeration Form enteries.
                    if (in_array('error_exists_email',        $error_codes)) return $email_exists;
        ...
        public function change_welcome_message($message)
        {
            ...
            if (
                ...
                //
                //   if ( array_key_exists( 'reset_hint_message', $this->options ) && ! empty( $this->options['reset_hint_message'] ) ) {
                //
                //     $loginpress_message = $this->options['reset_hint_message'];
                //   }
                // }
        ...
        {
            add_filter('gettext', [$this, 'change_username_label'], 99, 3);
            add_filter('gettext', [$this, 'change_password_label'], 99, 3);
            add_filter('gettext', [$this, 'change_rememberme_label'], 99, 3);
            add_filter('gettext', [$this, 'change_login_label'], 99, 3);
            add_filter('gettext', [$this, 'change_register_login_link_text'], 99, 3);
        }
        ...
        {
            add_filter('gettext', [$this, 'change_register_username_label'], 99, 3);
            add_filter('gettext', [$this, 'change_register_email_label'], 99, 3);
            add_filter('gettext', [$this, 'change_register_register_label'], 99, 3);
            add_filter('gettext', [$this, 'change_register_confirmation_text'], 99, 3);
            add_filter('gettext', [$this, 'change_login_register_link_text'], 99, 3);
        }

        // Check Lost Password page texts
        public function check_lostpasswords_texts()
        {
            add_filter('gettext', [$this, 'change_lostpasswords_username_label'], 99, 3);
            add_filter('gettext', [$this, 'change_lostpasswords_button_label'], 99, 3);
            add_filter('gettext', [$this, 'change_register_login_link_text'], 99, 3);
            add_filter('gettext', [$this, 'change_login_register_link_text'], 99, 3);
        }
        ...
        public function get_image_urls_by_ids($ids = '')
        {
            ...
            foreach ($image_ids as $image_id) {
                ...
                if (!empty($image_url)) $images[] = ['src' => $image_url];
            }
        ...
        {
            add_filter('gettext', [$this, 'change_lost_password_text'], 99, 3);
            add_filter('gettext_with_context', [$this, 'change_back_to_text'], 99, 4);
        }
