    ...
    class MenuEditor extends MenuEditorModel
    {
            ...
            if (is_array($menu_settings) && !array_key_exists(0, $menu_settings)) {
                foreach ($menu_settings as $key => $menu_s) {
                    $new_array = [
                        ...
                        $menu_s['icon'] ?? '',
                        'order' => $menu_s['order'],
                        'separator' => $menu_s['separator'] ?? 0,
                        'hidden_for' => $menu_s['hidden_for'] ?? []
                    ];
                    if (empty($this->options['admin_ui'])) {
                ...
                foreach ($menu_settings as $key => $menu_s) {
                    if (isset($menu_s['submenu']) && is_array($menu_s['submenu'])) {
                        ...
                        foreach ($menu_s['submenu'] as $k => $sub_menu) {
                            if (strpos($k, 'adminify-custom-submenu') === false) continue;
                            $order = $sub_menu['order'] ?? $i;
                            $new_array = [
                                $sub_menu['name'] ?? '',
                                'read',
                                $sub_menu['link'] ?? '',
                                'order' => $order,
                                'key' => $k,
                                'hidden_for' => $sub_menu['hidden_for'] ?? []
                            ];
        ...
        public function apply_sub_level_settings($subitems, $parentname)
        {
            ...
            foreach ($subitems as $current_menu_item) {
                if (empty($current_menu_item[0]) || (($current_menu_item[2] === 'wp-adminify-settings-pricing') && jltwp_adminify()->can_use_premium_code__premium_only())) continue;
                $name = '';
                $link = '';
                $disabled_for = array();
                $optiongroup = array();
        ...
        public function jltwp_adminify_menu_editor_submenu()
        {
            add_submenu_page(
                ...
                'adminify-menu-editor', // Page slug, will be displayed in URL
            ...
            if ($this->menu && is_array($this->menu)) {
                ...
                foreach ($this->menu as $menu_item) {
                    ...
                    if (strpos($menu_item[2], "separator") !== false  && !$menu_item[0]) {
                        ...
                        $this->render_top_level_menu_item($this->menu, $menu_item, $this->submenu, $next_menu_item);
                    }
                }
...
?>
                <script>
                    jQuery(function($) {
                        ...
                        $('.adminify-menu-settings').tokenize2({
                            placeholder: '<?php _e('Select roles or users', 'adminify') ?>'
                        });
         ...
         * @return void
         */
        public function render_top_level_menu_item($master_menu, $current_menu_item, $master_sub_menu, $next_menu_item)
        {
            ...
            $icons = array(
                ...
                // WP_ADMINIFY_ASSETS_IMAGE . 'logos/menu-icon.svg'        => WP_ADMINIFY_ASSETS_IMAGE . 'logos/menu-icon.svg'
            );

            $default_icons = (isset($current_menu_item)) ? $current_menu_item[6] : 'dashicons dashicons-external';

            ?>
            <div class="accordion adminify_menu_item" name="<?php echo esc_attr($name_attr); ?>" id="wp-adminify-top-menu-<?php echo esc_attr($menu_id); ?>">
                ...
                <a class="menu-editor-title accordion-button p-4" href="#">
                    ...
                    <?php echo (preg_replace('/\<span.*?>.*?\<\/span><\/span>/s', '', $current_menu_item[0])); ?>
                </a>
                ...
                <div class="accordion-body adminify_top_level_settings">
                    ...
                    <div class="tab-content tab-panel panel p-4">
                        <div id="tab-<?php echo esc_attr($menu_id); ?>-1" class="tab-pane">
                            <div class="menu-editor-form">
                                <div class="columns">
                                    ...
                                    <div class="column">
                                        <label for="<?php echo esc_attr($current_menu_item[2]); ?>">
                                            <?php _e('Hidden For Rules', 'adminify'); ?>
                                        </label>
                                        ...
                                        <div class="select is-small">
                                            <select class="adminify-menu-settings menu_setting" name="hidden_for" id="<?php echo esc_attr($menu_id); ?>-user-role-types" multiple>
                                                ...
                                                ?>
                                                <option value="Super Admin" <?php echo esc_attr($sel); ?>><?php _e('Super Admin', 'adminify') ?></option>
                                                <?php
                                                ...
                                                ?>
                                                    <option value="<?php echo esc_attr($username); ?>" <?php echo esc_attr($sel); ?>><?php echo esc_attr($username); ?></option>
                                                <?php
                                                }
                                                ?>
                                            </select>
                                ...
                                <div class="columns">
                                    <div class="column">
                                        <label for="">
                                            <?php _e('Change Link', 'adminify'); ?>
                                        </label>
                                        <input class="menu_setting" name="link" type="url" placeholder="<?php _e('New link', 'adminify'); ?>" value="<?php echo esc_attr(($name_attr != ltrim($link, '#')) ? $link : ''); ?>">
                                    </div>
                                    <div class="column">
                                        <label for="">
                                            <?php _e('Set Custom Icon', 'adminify'); ?>
                                        </label>
                                        <div class="icon-picker-wrap wp-adminify-menu-icon-picker adminify-icon-picker-input icon-select-button is-clickable is-pulled-left">
                                            <ul class="icon-picker">
                                                ...
                                                <li class="select-icon <?php echo ((!empty($icon) && preg_match('/http(s?)\:\/\//i', $icon)) || (empty($icon) && empty($icons[$default_icons]))) ? 'custom-icon' : ''; ?>" title="Icon Library">
                                                    <?php
                                                    if (!empty($icon)) {
                                                    ...
                                                    ?>
                                                            <i class="<?php echo $this->get_icon(esc_attr($icon), ''); ?>"></i>
                                                        <?php
                                                        }
                                                    } else {
                                ...
                                <div class='columns'>
                                    <div class='column <?php echo (!jltwp_adminify()->can_use_premium_code__premium_only()) ? 'upgrade-pro' : ''; ?>'>
                                        <?php if (jltwp_adminify()->can_use_premium_code__premium_only()) { ?>
                                            <label for="seperator-<?php echo esc_attr($name_attr); ?>">
                            ...
                            <?php if (strpos($current_menu_item[5], 'adminify-custom-menu-') !== false) { ?>
                                <div class="remove-add-new-menu"><span data-id="<?php echo esc_attr($current_menu_item[5]); ?>"><i class="icon-close"></i> Delete</span></div>
                            <?php } ?>
                        </div>
                        <div id="tab-<?php echo esc_attr($menu_id); ?>-2" class="tab-pane tab-pane--submenu">
                            ...
                            ?>
                                <span><?php _e('No sub menu items', 'adminify'); ?></span>
                            <?php
                            }

                            $this->render_add_new_menu_item(true); // true for submenu item
                            ?>
                ...
                <div class="accordion-body adminify_top_level_settings">
                    <div class="tab-content tab-panel panel p-4">
                        <div class="menu-editor-form">
                            <div class="columns">
                                <div class="column">
                                    <label for=""><?php _e('Rename as', 'adminify'); ?></label>
                                    <input class="menu_setting" type="text" name="name" placeholder="<?php _e('New Name', 'adminify'); ?>" value="<?php echo esc_attr($name); ?>">
                                </div>
                                <div class="column">
                                    <label for=""><?php _e('Hidden For Rules', 'adminify'); ?></label>
        ...
        ?>
            <div class="accordion adminify_sub_menu_item" name="<?php echo esc_attr($sub_menu_name_attr); ?>" id="wp-adminify-sub-menu-<?php echo esc_attr($menu_id); ?>">
                <input type="number" class="top_level_order" value="" style="display:none;">
                <a class="menu-editor-title accordion-button p-4" href="#">
                    <svg class="drag-icon is-pulled-left mr-2" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M12 14C13.1046 14 14 13.1046 14 12C14 10.8954 13.1046 10 12 10C10.8954 10 10 10.8954 10 12C10 13.1046 10.8954 14 12 14Z" fill="#4E4B66" fill-opacity="0.72" />
                        ...
                        <div class="tab-pane">
                            <div class="menu-editor-form">
                                <div class="columns">
                                    <div class="column">
                                        <label for=""><?php _e('Rename as', 'adminify'); ?></label>
                                        <input class="sub_menu_setting" type="text" data-sub-menu-id="<?php echo esc_attr($menu_id); ?>" name="name" placeholder="<?php _e('New Menu name...', 'adminify') ?>" value="<?php echo esc_attr($name); ?>">
                                    </div>
                                    <div class="column">
                                        <label for=""><?php _e('Hidden For Rules', 'adminify'); ?></label>
                                        ...
                                        <div class="select is-small">
                                            ...
                                            <script>
                                                jQuery('#wp-adminify-sub-menu-<?php echo esc_attr($menu_id); ?> #<?php echo esc_attr($menu_id); ?>-user-role-types').tokenize2({
                                                    placeholder: '<?php _e('Select roles or users', 'adminify') ?>'
                                                });
                                                jQuery(document).ready(function($) {
                                                    $('#wp-adminify-sub-menu-<?php echo esc_attr($menu_id); ?> #<?php echo esc_attr($menu_id); ?>-user-role-types').on('tokenize:select', function(container) {
                                                        $(this).tokenize2().trigger('tokenize:search', [$(this).tokenize2().input.val()]);
                                                    });
                                                })
                                            </script>
                                ...
                                <div class="columns">
                                    <div class="column">
                                        <label for=""><?php _e('Change Link', 'adminify'); ?></label>
                                        <input class="sub_menu_setting" name="link" type="url" placeholder="New link" value="<?php echo esc_attr(($sub_menu_name_attr != ltrim($link, '#')) ? $link : ''); ?>">
                                    </div>
                                    <div class="column">
                                        <label for=""><?php _e('Set Custom Icon', 'adminify'); ?> <i>(<?php _e('Not available for Submenu.', 'adminify'); ?>)</i></label>
                                        ...
                                        <div class="icon-picker-wrap wp-adminify-menu-icon-picker adminify-icon-picker-input icon-select-button is-clickable-no is-pulled-left">
                                            <ul class="icon-picker">
                                                ...
                                                if (strpos($sub_menu_name_attr, 'adminify-custom-submenu-') !== false) {
                                                    ...
                                                    $icon = '<img src="' . WP_ADMINIFY_ASSETS_IMAGE . 'logos/menu-icon.svg" />';
                                                }
                                                ?>
                                                <li class="select-icon <?php echo esc_attr($custom_class); ?>" title="Icon Library">
                                                    <?php echo $icon; ?>
                                                </li>
                            ...
                            <?php if (strpos($sub_menu_name_attr, 'adminify-custom-submenu-') !== false) { ?>
                                <div class="remove-add-new-menu"><span data-id="<?php echo esc_attr($sub_menu_name_attr); ?>"><i class="icon-close"></i> Delete</span></div>
                            <?php } ?>
                        </div>
                    </div>
                </div>
        ...
        {
        ?>
            <div class="accordion add-new-menu-editor-item <?php echo esc_attr($submenu ? 'submenu' : ''); ?> <?php echo (!jltwp_adminify()->can_use_premium_code__premium_only()) ? 'upgrade-pro' : ''; ?>">
        ...
        ?>
            ...
            <div class="is-inline-block">
                <p><?php _e('Edit each menu item\'s name, link, icon and visibility. Drag and drop to rearange the menu. Changes will take effect after page refresh.', 'adminify'); ?></p>
                <p>
                    ...
                    <?php _e('If you have WP Adminify Menu Module disabled, icons and label dividers won\'t change.', 'adminify'); ?>
