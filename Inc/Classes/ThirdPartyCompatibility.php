...
class ThirdPartyCompatibility
{
    ...
    public function jltwp_adminify_reset_theme_conflicts()
    {
        ...
        if ('Neve' == $theme->name || 'Neve' == $theme->parent_theme) {
            wp_enqueue_style('wp-adminify_neve-theme', WP_ADMINIFY_ASSETS . 'css/themes/neve.css', false, WP_ADMINIFY_VER);
        }
    ...
    public function register_compatability_styles($plugin_supports)
    {
        ...
        if (jltwp_adminify()->can_use_premium_code__premium_only()) {
            ...
            $plugin_dir = WP_ADMINIFY_URL . 'Pro/assets/css/plugins/';
            $plugin_files = list_files(WP_ADMINIFY_PATH . 'Pro/assets/css/plugins/', 1);
                ...
                if (is_plugin_active_for_network($active_path)) {
                    ...
                    if (isset($plugin_supports[$pluginname])) {
                        if ($plugin_supports[$pluginname] != "") {
                            wp_register_style('wp-adminify_site-wide_' . $pluginname . '_css', $plugin_supports[$pluginname], array(), WP_ADMINIFY_VER);
