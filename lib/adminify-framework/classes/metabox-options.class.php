 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'ADMINIFY_Metabox' ) ) {
	class ADMINIFY_Metabox extends ADMINIFY_Abstract {
		...
		public function __construct( $key, $params = [] ) {
			...
			add_action( 'edit_attachment', [ $this, 'save_meta_box' ] );

			if ( ! empty( $this->page_templates ) || ! empty( $this->post_formats ) || ! empty( $this->args['class'] ) ) {
				foreach ( $this->post_type as $post_type ) {
					add_filter( 'postbox_classes_' . $post_type . '_' . $this->unique, [ $this, 'add_metabox_classes' ] );
				}
			}
		...
		public function add_metabox_classes( $classes ) {
			...
			if ( ! empty( $this->post_formats ) ) {
				...
				foreach ( $this->post_formats as $format ) {
					$classes[] = 'adminify-post-format-' . $format;
				}

				if ( ! in_array( $saved_post_format, $this->post_formats ) ) {
					$classes[] = 'adminify-metabox-hide';
				} else {
					$classes[] = 'adminify-metabox-show';
				}
			}
			...
			if ( ! empty( $this->page_templates ) ) {
				...
				$classes[] = 'adminify-page-templates';

				foreach ( $this->page_templates as $template ) {
					$classes[] = 'adminify-page-' . preg_replace( '/[^a-zA-Z0-9]+/', '-', strtolower( $template ) );
				}
		...
		public function add_meta_box( $post_type ) {
			if ( ! in_array( $post_type, $this->args['exclude_post_types'] ) ) {
				add_meta_box( $this->unique, $this->args['title'], [ $this, 'add_meta_box_content' ], $this->post_type, $this->args['context'], $this->args['priority'], $this->args );
			}
		}
		...
		public function add_meta_box_content( $post, $callback ) {
			...
			$errors    = ( is_object( $post ) ) ? get_post_meta( $post->ID, '_adminify_errors_' . $this->unique, true ) : [];
			$errors    = ( ! empty( $errors ) ) ? $errors : [];
			$theme     = ( $this->args['theme'] ) ? ' adminify-theme-' . $this->args['theme'] : '';
			$nav_type  = ( $this->args['nav'] === 'inline' ) ? 'inline' : 'normal';

			if ( is_object( $post ) && ! empty( $errors ) ) {
				delete_post_meta( $post->ID, '_adminify_errors_' . $this->unique );
			}

			wp_nonce_field( 'adminify_metabox_nonce', 'adminify_metabox_nonce' . $this->unique );

			echo '<div class="adminify adminify-metabox' . esc_attr( $theme ) . '">';
			...
			foreach ( $this->sections as $section ) {
				...
				if ( ! empty( $section['fields'] ) ) {
					foreach ( $section['fields'] as $field ) {
						...
						ADMINIFY::field( $field, $this->get_meta_value( $field ), $this->unique, 'metabox' );
					}
				} else {
		...
		public function save_meta_box( $post_id ) {
			...
			$data     = [];
			$errors   = [];
			$noncekey = 'adminify_metabox_nonce' . $this->unique;
			$nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';

			if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'adminify_metabox_nonce' ) ) {
				return $post_id;
			}
			...
			$data = apply_filters( "adminify_{$this->unique}_save", $data, $post_id, $this );

			do_action( "adminify_{$this->unique}_save_before", $data, $post_id, $this );
			...
			if ( empty( $data ) || ! empty( $request['_reset'] ) ) {
				...
				if ( ! empty( $errors ) ) {
					update_post_meta( $post_id, '_adminify_errors_' . $this->unique, $errors );
				}
			}

			do_action( "adminify_{$this->unique}_saved", $data, $post_id, $this );

			do_action( "adminify_{$this->unique}_save_after", $data, $post_id, $this );
		}
	}
}
