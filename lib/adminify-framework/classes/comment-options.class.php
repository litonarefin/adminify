	...
	class ADMINIFY_Comment_Metabox extends ADMINIFY_Abstract {
		...
		public function __construct( $key, $params = [] ) {
			...
			if ( ! empty( $this->args['class'] ) ) {
				add_filter( 'postbox_classes_comment_' . $this->unique, [ $this, 'add_comment_metabox_classes' ] );
			}
		}
		...
		public function add_comment_meta_box( $post_type ) {
			add_meta_box( $this->unique, $this->args['title'], [ $this, 'add_comment_meta_box_content' ], 'comment', 'normal', $this->args['priority'], $this->args );
		}
		...
		public function add_comment_meta_box_content( $comment, $callback ) {
			...
			$errors   = ( is_object( $comment ) ) ? get_comment_meta( $comment->comment_ID, '_adminify_errors_' . $this->unique, true ) : [];
