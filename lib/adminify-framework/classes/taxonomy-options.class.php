 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'ADMINIFY_Taxonomy_Options' ) ) {
	class ADMINIFY_Taxonomy_Options extends ADMINIFY_Abstract {
		...
		public function __construct( $key, $params ) {
			...
			if ( ! empty( $this->taxonomies ) && in_array( $this->taxonomy, $this->taxonomies ) ) {
				add_action( 'admin_init', [ $this, 'add_taxonomy_options' ] );
			}
		...
		public function add_taxonomy_options() {
			add_action( $this->taxonomy . '_add_form_fields', [ $this, 'render_taxonomy_form_fields' ] );
			add_action( $this->taxonomy . '_edit_form', [ $this, 'render_taxonomy_form_fields' ] );

			add_action( 'created_' . $this->taxonomy, [ $this, 'save_taxonomy' ] );
			add_action( 'edited_' . $this->taxonomy, [ $this, 'save_taxonomy' ] );
		}
		...
		public function render_taxonomy_form_fields( $term ) {
			...
			$errors    = ( ! empty( $term_id ) ) ? get_term_meta( $term_id, '_adminify_errors_' . $this->unique, true ) : [];
			$errors    = ( ! empty( $errors ) ) ? $errors : [];
			$class     = ( $this->args['class'] ) ? ' ' . $this->args['class'] : '';

			if ( ! empty( $errors ) ) {
				delete_term_meta( $term_id, '_adminify_errors_' . $this->unique );
			}

			wp_nonce_field( 'adminify_taxonomy_nonce', 'adminify_taxonomy_nonce' . $this->unique );

			echo '<div class="adminify adminify-taxonomy adminify-show-all adminify-onload adminify-taxonomy-' . esc_attr( $classname ) . '-fields ' . esc_attr( $class ) . '">';
				...
				if ( $taxonomy === $this->taxonomy ) {
					...
					if ( ! empty( $section['fields'] ) ) {
						foreach ( $section['fields'] as $field ) {
							...
							ADMINIFY::field( $field, $this->get_meta_value( $field, $term_id ), $this->unique, 'taxonomy' );
						}
					}
				}
			}
		...
		public function save_taxonomy( $term_id ) {
			...
			$data     = [];
			$errors   = [];
			$noncekey = 'adminify_taxonomy_nonce' . $this->unique;
			$nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';
			$taxonomy = ( ! empty( $_POST['taxonomy'] ) ) ? sanitize_text_field( wp_unslash( $_POST['taxonomy'] ) ) : '';

			if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'adminify_taxonomy_nonce' ) ) {
				return $term_id;
			}
			...
			$data = apply_filters( "adminify_{$this->unique}_save", $data, $term_id, $this );

			do_action( "adminify_{$this->unique}_save_before", $data, $term_id, $this );
			...
			if ( empty( $data ) ) {
				...
				if ( ! empty( $errors ) ) {
					update_term_meta( $term_id, '_adminify_errors_' . $this->unique, $errors );
				}
			}

			do_action( "adminify_{$this->unique}_saved", $data, $term_id, $this );

			do_action( "adminify_{$this->unique}_save_after", $data, $term_id, $this );
		}
	}
}
