 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'ADMINIFY_Nav_Menu_Options' ) ) {
	class ADMINIFY_Nav_Menu_Options extends ADMINIFY_Abstract {
		...
		public function __construct( $key, $params ) {
			...
			add_filter( 'wp_edit_nav_menu_walker', [ $this, 'wp_edit_nav_menu_walker' ], 10, 2 );
		}
		...
		public function wp_edit_nav_menu_walker( $class, $menu_id ) {
			...
			if ( version_compare( $wp_version, '5.4.0', '<' ) ) {
				if ( ! class_exists( 'ADMINIFY_Walker_Nav_Menu_Edit' ) ) {
					ADMINIFY::include_plugin_file( 'functions/walker.php' );
				}

				return 'ADMINIFY_Walker_Nav_Menu_Edit';
			}
		...
		public function wp_nav_menu_item_custom_fields( $menu_item_id, $item, $depth, $args ) {
			$errors = ( ! empty( $menu_item_id ) ) ? get_post_meta( $menu_item_id, '_adminify_errors_' . $this->unique, true ) : [];
			$errors = ( ! empty( $errors ) ) ? $errors : [];
			$class  = ( $this->args['class'] ) ? ' ' . $this->args['class'] : '';

			if ( ! empty( $errors ) ) {
				delete_post_meta( $menu_item_id, '_adminify_errors_' . $this->unique );
			}

			echo '<div class="adminify adminify-nav-menu-options' . esc_attr( $class ) . '">';

			foreach ( $this->sections as $section ) {
				$section_icon  = ( ! empty( $section['icon'] ) ) ? '<i class="adminify-nav-menu-icon ' . esc_attr( $section['icon'] ) . '"></i>' : '';
				$section_title = ( ! empty( $section['title'] ) ) ? $section['title'] : '';

				echo '<div class="adminify-fields">';

				echo ( $section_title || $section_icon ) ? '<div class="adminify-nav-menu-title"><h4>' . $section_icon . $section_title . '</h4></div>' : '';
				...
				if ( ! empty( $section['fields'] ) ) {
					foreach ( $section['fields'] as $field ) {
						...
						ADMINIFY::field( $field, $this->get_meta_value( $menu_item_id, $field ), $this->unique . '[' . $menu_item_id . ']', 'menu' );
					}
				}
		...
		public function wp_update_nav_menu_item( $menu_id, $menu_item_db_id, $menu_item_args ) {
			...
			$request = ( ! empty( $_POST[ $this->unique ][ $menu_item_db_id ] ) ) ? $_POST[ $this->unique ][ $menu_item_db_id ] : [];
			...
			$data = apply_filters( "adminify_{$this->unique}_save", $data, $menu_item_db_id, $this );

			do_action( "adminify_{$this->unique}_save_before", $data, $menu_item_db_id, $this );
			...
			if ( empty( $data ) ) {
				...
				if ( ! empty( $errors ) ) {
					update_post_meta( $menu_item_db_id, '_adminify_errors_' . $this->unique, $errors );
				}
			}

			do_action( "adminify_{$this->unique}_saved", $data, $menu_item_db_id, $this );

			do_action( "adminify_{$this->unique}_save_after", $data, $menu_item_db_id, $this );
		}
