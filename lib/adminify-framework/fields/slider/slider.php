...
if ( ! class_exists( 'ADMINIFY_Field_slider' ) ) {
	class ADMINIFY_Field_slider extends ADMINIFY_Fields {
		...
		public function render() {
			...
			echo '<input type="number" name="' . esc_attr( $this->field_name() ) . '" value="' . esc_attr( $this->value ) . '"' . $this->field_attributes( [ 'class' => 'adminify-input-number' . esc_attr( $is_unit ) ] ) . ' data-min="' . esc_attr( $args['min'] ) . '" data-max="' . esc_attr( $args['max'] ) . '" data-step="' . esc_attr( $args['step'] ) . '" step="any" />';
