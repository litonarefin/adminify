	...
	***************************************************************************** */
	...
	function FlatpickrInstance(element, instanceConfig) {
		...
		function buildTime() {
			...
			var hourInput               = createNumberInput(
				"flatpickr-hour",
				{
			...
			var minuteInput             = createNumberInput(
				"flatpickr-minute",
				{
			...
			if (self.isOpen && ! self.config.inline) {
				...
				var lostFocus = e.type === "blur"
					...
					: ! isInput &&
						! isCalendarElement &&
						! isCalendarElem( e.relatedTarget );
				var isIgnored = ! self.config.ignoredFocusElements.some(
					function (elem) {
		...
		function parseConfig() {
			...
			HOOKS.filter( function (hook) { return self.config[hook] !== undefined; } ).forEach(
				function (hook) {
			...
			return arr
				...
				.map(
					function (rule) {
						...
						rule.from &&
						rule.to) {
							return from: self.parseDate( rule.from, undefined ),
...
(function($) {
	...
	function ADMINIFYAjaxChosen(element, options) {
		this.element = $( element );
		this.options = options;
		this.init();
	};

	ADMINIFYAjaxChosen.prototype.init = function() {
	...
	ADMINIFYAjaxChosen.prototype.events = function() {
	...
	ADMINIFYAjaxChosen.prototype.search_field_focused = function() {
	...
	ADMINIFYAjaxChosen.prototype.search_welcome_message = function() {
	...
	ADMINIFYAjaxChosen.prototype.update_list = function() {
		...
		this.timer = setTimeout(
			function() {
				...
				_this.chosenXhr = window.wp.ajax.post( 'adminify-chosen', _this.options.data ).done(
					function( response ) {
	...
	ADMINIFYAjaxChosen.prototype.show_results = function( items ) {
		...
		if ( this.is_multiple ) {
			...
			var $hidden_select = this.element.parent().find( '.adminify-hide-select' );
			var $hidden_value  = $hidden_select.val() || [];

			this.element.ADMINIFYChosenOrder( $hidden_value, true );
	...
	$.fn.ADMINIFYAjaxChosen = function(chosenOptions) {
		return this.each(
			function() {
				new ADMINIFYAjaxChosen( this, chosenOptions );
			}
		);
	};
...
(function() {
	...
	ADMINIFYAbstractChosenOrder = (function() {

		function ADMINIFYAbstractChosenOrder() {}

		ADMINIFYAbstractChosenOrder.insertAt = function(node, index, parentNode) {
			return parentNode.insertBefore( node, parentNode.children[index].nextSibling );
		};

		ADMINIFYAbstractChosenOrder.getFlattenedOptionsAndGroups = function(select) {
			var flattened_options, opt, options, sub_opt, sub_options, _i, _j, _len, _len1;
			options           = Array.prototype.filter.call(
				select.childNodes,
				function(o) {
					var _ref;
		...
		ADMINIFYAbstractChosenOrder.isValidMultipleSelectElement = function(element) {
			return element !== null && typeof element !== "undefined" && element.nodeName === "SELECT" && element.multiple;
		};

		ADMINIFYAbstractChosenOrder.getChosenUIContainer = function(select) {
			if (select.id !== "") {
		...
		ADMINIFYAbstractChosenOrder.isChosenified = function(select) {
			return this.getChosenUIContainer( select ) != null;
		};

		ADMINIFYAbstractChosenOrder.forceSelection = function(select, selection) {
		...
		ADMINIFYAbstractChosenOrder.ADMINIFYChosenOrder = function(select, order, force) {
			...
			if (order instanceof Array) {
				...
				for (i = _i = 0, _len = order.length; _i < _len; i = ++_i) {
					...
					option           = Array.prototype.filter.call(
						chosen_options,
						function(o) {
		...
		return ADMINIFYAbstractChosenOrder;
	...
	$.fn.extend(
		{
			ADMINIFYChosenOrder: function(order, force) {
				return _ADMINIFYChosenOrder.ADMINIFYChosenOrder( this, order, force );
			}
		}
	);

	this._ADMINIFYChosenOrder = (function(_super) {
		__extends( _ADMINIFYChosenOrder, _super );

		function _ADMINIFYChosenOrder() {
			_ref = _ADMINIFYChosenOrder.__super__.constructor.apply( this, arguments );
			return _ref;
		}

		_ADMINIFYChosenOrder.relAttributeName = 'data-option-array-index';

		_ADMINIFYChosenOrder.isjQueryObject = function(obj) {
			return (typeof jQuery !== "undefined" && jQuery !== null) && obj instanceof jQuery;
		};

		_ADMINIFYChosenOrder.getDOMElement = function(element) {
		...
		_ADMINIFYChosenOrder.searchChosenUIContainer = function(element) {
		...
		_ADMINIFYChosenOrder.triggerEvent = function(target, event_name) {
			return $( target ).trigger( event_name );
		};

		return _ADMINIFYChosenOrder;

	})( ADMINIFYAbstractChosenOrder );
		...
		Rule.prototype,
		{
			...
			applyRule: function(context, enforced) {
				...
				if ( result ) {
					...
					$( controls ).each(
						function() {
							  $( this ).removeClass( 'adminify-depend-on' );
						}
					);
					...
					$( controls ).each(
						function() {
							$( this ).addClass( 'adminify-depend-on' );
						}
					);
...
(function(root, factory) {
	...
	//
	// Adminify: Added custom patterns for spesific validate
	//
	var patterns = {
	...
	//
	// Adminify: Renamed function names for avoid conflicts
	//

	if (typeof $.fn !== "undefined") {
		$.fn.serializeObjectADMINIFY = FormSerializer.serializeObject;
		$.fn.serializeJSONADMINIFY   = FormSerializer.serializeJSON;
	}
