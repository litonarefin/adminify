 ...
 */(function( $, window, document, undefined ) {
	...
	$.fn.adminify_nav_options = function() {
		return this.each(
			function() {
				...
				$window.on(
					'hashchange adminify.hashchange',
					function() {
					...
					if ( wp.customize.notifications && wp.customize.OverlayNotification ) {
						...
						wp.customize.notifications.add(
							new wp.customize.OverlayNotification(
								'adminify_field_backup_notification',
								{
									type: 'default',
									message: '&nbsp;',
									loading: true
	...
	$.fn.adminify_field_gallery = function() {
		return this.each(
			function() {
				...
				$this.on(
					'click',
					'.adminify-button, .adminify-edit-gallery',
					function( e ) {
		...
		return this.each(
			function() {
				...
				var update_title_numbers = function( $selector ) {
					$selector.find( '.adminify-cloneable-title-number' ).each(
						function( index ) {
				...
				$group.children( '.adminify-cloneable-add' ).on(
					'click',
					function( e ) {
						...
						$cloned_item.find( ':input[name!="_pseudo"]' ).each(
							function() {
						...
						if ( ! ADMINIFY.vars.icon_modal_loaded ) {
							...
							window.wp.ajax.post(
								...
								function( response ) {
									...
									$modal.on(
										'click',
										'.adminify-modal-close, .adminify-modal-overlay',
										function() {
											$modal.addClass( 'hidden' );
										}
									);
	...
	$.fn.adminify_field_repeater = function() {
		return this.each(
			function() {
				...
				$repeater.children( '.adminify-repeater-add' ).on(
					'click',
					function( e ) {
					...
					function( e ) {
						...
						if ( ! flooding ) {
							...
							if ( $this.hasClass( 'adminify-save-ajax' ) ) {
								...
								window.wp.ajax.post(
									'adminify_' + $panel.data( 'unique' ) + '_ajax_save',
									{
								...
								.done(
									function( response ) {
										...
										$result_success.empty().append( response.notice ).fadeIn(
											'fast',
											function() {
												timeout = setTimeout(
													function() {
	...
	$.fn.adminify_shortcode = function() {
		...
		base.shortcode_parse = function( serialize, key ) {
			...
			$.each(
				serialize,
				function( shortcode_key, shortcode_values ) {
		...
		return this.each(
			function() {
				...
				$( document ).on(
					'click',
					'.adminify-shortcode-button[data-modal-id="' + modal_id + '"]',
					function( e ) {
				...
				$select.on(
					...
					function() {
						...
						if ( sc_key ) {
							...
							window.wp.ajax.post(
								'adminify-get-shortcode-' + modal_id,
								{
				...
				$input.wpColorPicker(
					{
						...
						create: function() {
							...
							$transparent_wrap    = $(
								'<div class="adminify--transparent-wrap">' +
					...
					$chosen_choices.sortable(
						{
							...
							update: function( e, ui ) {
								...
								$chosen_choices.find( '.search-choice-close' ).each(
									function() {
										var option_array_index = $( this ).data( 'option-array-index' );
										$.each(
											chosen_object.results_data,
											function(index, data) {
	...
	// Window on resize
	//
	ADMINIFY.vars.$window.on(
		'resize adminify.resize',
		ADMINIFY.helper.debounce(
			function( event ) {
