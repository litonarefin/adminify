							...
							<div class="fs-header-actions">
								<ul>
									<?php if ( $fs->is_whitelabeled( true ) ) : ?>
										<li>
											<a href="#" class="debug-license-trigger"><i class="dashicons dashicons-<?php echo esc_attr( $is_whitelabeled ? 'editor-code' : 'controls-pause' ); ?>"></i> <span>
																															   <?php
																																if ( $is_whitelabeled ) {
										...
										<?php if ( ! $is_paying ) : ?>
											<li>
												<form action="<?php echo esc_url($fs->_get_admin_page_url( 'account' )); ?>" method="POST">
													...
													<a class="fs-delete-account" href="#" onclick="if (confirm('
													<?php
									...
									foreach ( $profile as $p ) :
										?>
											...
											<tr class="fs-field-<?php echo esc_attr( $p['id'] ); ?>
												...
												?>
												>
												<?php if ( in_array( $p['id'], [ 'license_key', 'site_secret_key' ] ) ) : ?>
													...
													<?php if ( 'plan' === $p['id'] ) : ?>
														...
														<?php if ( is_object( $license ) && ! $license->is_lifetime() ) : ?>
															<?php if ( ! $is_active_subscription && ! $is_active_bundle_subscription && ! $license->is_first_payment_pending() ) : ?>
																...
																<label
																	class="fs-tag <?php echo esc_attr($is_license_expired ? 'fs-error' : 'fs-warn'); ?>">
																		<?php
													...
													<?php elseif ( 'version' === $p['id'] && $has_paid_plan ) : ?>
														<?php if ( $fs->has_premium_version() ) : ?>
															<?php if ( $is_premium ) : ?>
																<label
																	class="fs-tag fs-<?php echo esc_attr($fs->can_use_premium_code() ? 'success' : 'warn'); ?>"><?php fs_esc_html_echo_inline( 'Premium version', 'premium-version', $slug ); ?></label>
												...
												<?php if ( 'plan' !== $p['id'] && 'bundle_plan' !== $p['id'] ) : ?>
													<td class="fs-right">
														...
														<?php if ( 'version' === $p['id'] ) : ?>
																	...
																	<?php if ( $is_paying || $fs->is_trial() ) : ?>
																		<?php if ( ! $fs->is_allowed_to_install() ) : ?>
																			<a target="_blank" rel="noopener" class="button button-primary"
																				href="<?php echo esc_url($fs->_get_latest_download_local_url()); ?>">
																								 <?php
																								 ...
																								 echo wp_kses_post( $download_version_text );
																									?>
																			</a>
															...
															<?php endif ?>
															<?php
														elseif ( in_array( $p['id'], [ 'license_key', 'site_secret_key' ] ) ) :
															?>
															<?php if ( ! $is_whitelabeled ) : ?>
														...
														( is_string( $user->secret_key ) && in_array(
															...
															<form action="<?php echo esc_attr($fs->_get_admin_page_url( 'account' )); ?>" method="POST"
																  ...
																  <?php
																	...
																	?>
																  ', '<?php echo esc_js($p['value']); ?>'); if (null == val || '' === val) return false; jQuery('input[name=fs_<?php echo esc_js($p['id']); ?>_<?php echo esc_js($fs->get_unique_affix()); ?>]').val(val); return true;">
