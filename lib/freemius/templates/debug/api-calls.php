...
<?php foreach ( $counters as $method => $count ) : ?>
	<h2><span><?php echo Utils::wp_kses_custom($method); ?>:</span><?php echo number_format( $count ); ?></h2>
<?php endforeach ?>
<table class="widefat">
	<thead>
	<tr>
	...
	<?php foreach ( $logger as $log ) : ?>
		<tr>
			<td><?php echo Utils::wp_kses_custom($log['id']); ?>.</td>
			<td><?php echo Utils::wp_kses_custom($log['method']); ?></td>
			<td><?php echo Utils::wp_kses_custom($log['code']); ?></td>
			...
			<td>
				...
				if ( $is_not_empty_result ) :
					?>
					 style="display: none"<?php endif ?>><code><?php echo esc_html( $result ); ?></code></pre>
			</td>
