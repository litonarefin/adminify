...
<script type="text/javascript">
	(function ($) {
		$(document).ready(function () {
			...
			$tab.addClass('fs-tab');
			$tab.addClass('<?php echo esc_js($fs->get_unique_affix()); ?>');
			...
			$tabClone.html(<?php echo json_encode( $tab['label'] ); ?>)
				.attr('href', '<?php echo esc_js($is_support_tab ? $fs->get_support_forum_url() : $tab['href']); ?>')
				.appendTo($tabsWrapper)
				// Remove any custom click events.
				.off('click', '**')
				.addClass('<?php echo esc_js($tab['slug']); ?>')
			...
			$tabClone.addClass('nav-tab-active');
						...
						<?php if ( in_array( $freemius_context_page, [ 'pricing', 'contact', 'checkout' ] ) ) : ?>
			// Add AJAX loader.
...
//						window.scrollTo(0,0);
			...
			if (window.location.hash) {
				...
				<?php
				if ( is_string( $freemius_context_page ) && in_array(
					$freemius_context_page,
					[
						'pricing',
						'contact',
						'checkout',
