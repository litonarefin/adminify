	...
	(function ($) {
		$(document).ready(function () {
			var contentHtml      = <?php echo json_encode( $modal_content_html ); ?>,
				modalHtml        =
					'<div class="fs-modal fs-modal-license-key-resend <?php echo esc_js($is_freemium) ? 'fs-freemium' : 'fs-premium'; ?>">'
			...
			function registerEventHandlers() {
				$('a.show-license-resend-modal-<?php echo esc_js($fs->get_unique_affix()); ?>').click(function (evt) {
					evt.preventDefault();
				...
				$modal.on('click', '.button', function (evt) {
					...
					$.ajax({
						...
						method    : 'POST',
						data      : {
							action     : '<?php echo esc_js($fs->get_ajax_action( 'resend_license_key' )); ?>',
							security   : '<?php echo esc_js($fs->get_ajax_security( 'resend_license_key' )); ?>',
							module_id  : moduleID,
							email      : email
						},
						beforeSend: function () {
