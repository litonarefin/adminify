		...
		$( document ).ready(function() {
			var modalContentHtml = <?php echo json_encode( $modal_content_html ); ?>,
				modalHtml =
					...
					+ '			<button class="button <?php echo esc_js($fs->is_premium()) ? 'button-primary warn' : 'button-secondary'; ?> button-opt-out" tabindex="1"><?php echo esc_js( $opt_out_text ); ?></button>'
					+ '			<button class="button <?php echo esc_js($fs->is_premium()) ? 'button-secondary' : 'button-primary'; ?> button-close" tabindex="2"><?php echo esc_js( $primary_cta_label ); ?></button>'
					+ '		</div>'
					+ '	</div>'
					+ '</div>',
				$modal              = $(modalHtml),
				...
				actionLinkSelector  = 'span.opt-in-or-opt-out.<?php echo esc_js($slug); ?> a',
				...
				$body               = $( 'body' ),
				moduleID            = '<?php echo esc_js($fs->get_id()); ?>';
				...
				$.ajax({
					...
					data: {
						action   : ( 'stop_tracking' == action ?
								'<?php echo esc_js($fs->get_ajax_action( 'stop_tracking' )); ?>' :
								'<?php echo esc_js($fs->get_ajax_action( 'allow_tracking' )); ?>'
						),
						security : ( 'stop_tracking' == action ?
								'<?php echo esc_js($fs->get_ajax_security( 'stop_tracking' )); ?>' :
								'<?php echo esc_js($fs->get_ajax_security( 'allow_tracking' )); ?>'
						),
						module_id: moduleID,
						_wp_http_referer: '<?php echo esc_js($fs->current_page_url()); ?>'
					},
					beforeSend: function() {
			...
			$extensionsTracking.on('click', function() {
				...
				$.ajax({
					...
					method: 'POST',
					data: {
						action    : '<?php echo esc_js($fs->get_ajax_action( 'update_tracking_permission' )); ?>',
						security  : '<?php echo esc_js($fs->get_ajax_security( 'update_tracking_permission' )); ?>',
						module_id : moduleID,
						_wp_http_referer: '<?php echo esc_js($fs->current_page_url()); ?>',
