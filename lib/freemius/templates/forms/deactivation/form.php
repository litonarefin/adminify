...
(function ($) {
	var reasonsHtml                    = <?php echo json_encode( $reasons_list_items_html ); ?>,
		modalHtml                      =
			...
			+ '			<div class="fs-modal-panel" data-panel-id="confirm"><p><?php echo esc_js($confirmation_message); ?></p></div>'
			...
			+ '         <?php echo esc_js($anonymous_feedback_checkbox_html); ?>'
		...
		deleteThemeUpdateData          = <?php echo esc_js($fs->is_theme() && $fs->is_premium() && ! $fs->has_any_active_valid_license() ? 'true' : 'false'); ?>,
		$subscriptionCancellationModal = $( '.fs-modal-subscription-cancellation-<?php echo esc_js($fs->get_id()); ?>' ),
		...
		$subscriptionCancellationModal.on( '<?php echo esc_js($fs->get_action_tag( 'subscription_cancellation_action' )); ?>', function( evt, cancelSubscription ) {
			...
			if ( false === cancelSubscription ) {
				...
				$.ajax({
					...
					method    : 'POST',
					data      : {
						action   : '<?php echo esc_js($fs->get_ajax_action( 'cancel_subscription_or_trial' )); ?>',
						security : '<?php echo esc_js($fs->get_ajax_security( 'cancel_subscription_or_trial' )); ?>',
					...
					success: function( result ) {
						if ( result.success ) {
							...
							$subscriptionCancellationModal.find( '.fs-modal-footer .button-primary' ).html( 
							<?php
							echo json_encode(
								sprintf(
	...
	function registerEventHandlers() {
		$body.on( 'click', '#the-list .deactivate > a', function ( evt ) {
			if ( 0 === $( this ).next( '[data-module-id=<?php echo esc_js($fs->get_id()); ?>].fs-module-id' ).length ) {
				return true;
			}
		...
		if ( ! $fs->is_plugin() ) {
			 ...
			 */
			?>
		$('body').on('click', '.theme-browser .theme:not([data-slug=<?php echo esc_js($fs->get_premium_slug()); ?>]) .theme-actions .button.activate', function (evt) {
			evt.preventDefault();
		...
		$modal.on('click', '.fs-modal-footer .button', function (evt) {
			...
			if (_this.hasClass('allow-deactivate')) {
				...
				if (0 === $radio.length) {
					if ( ! deleteThemeUpdateData ) {
						...
						$.ajax({
							...
							method    : 'POST',
							data      : {
								action   : '<?php echo esc_js($fs->get_ajax_action( 'delete_theme_update_data' )); ?>',
								security : '<?php echo esc_js($fs->get_ajax_security( 'delete_theme_update_data' )); ?>',
								module_id: '<?php echo esc_js($fs->get_id()); ?>'
							},
							beforeSend: function() {
				...
				$.ajax({
					...
					method    : 'POST',
					data      : {
						action      : '<?php echo esc_js($fs->get_ajax_action( 'submit_uninstall_reason' )); ?>',
						security    : '<?php echo esc_js($fs->get_ajax_security( 'submit_uninstall_reason' )); ?>',
						module_id   : '<?php echo esc_js($fs->get_id()); ?>',
						reason_id   : $radio.val(),
