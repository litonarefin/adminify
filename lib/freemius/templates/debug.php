					...
					<tr>
						...
						if ( FS_Plugin_Plan::is_valid_id( $site->plan_id ) ) {
							if ( false === $all_plans ) {
								...
								$all_plans = fs_get_entities( $fs_options->get_option( $option_name, [] ), FS_Plugin_Plan::get_class_name() );
							}
							...
							foreach ( $all_plans[ $slug ] as $plan ) {
								...
								echo wp_kses_post( $plan_name );
						?>
							</td>
							<td><?php echo wp_kses_post( $site->public_key ); ?></td>
						<td>
						<?php
								$plugin_storage = FS_Storage::instance( $module_type, $slug );

								echo esc_html(
									$plugin_storage->is_whitelabeled ?
						...
						<td>
							<form action="" method="POST">
								...
								<input type="hidden" name="module_id" value="<?php echo esc_html( $site->plugin_id ); ?>">
								<?php if ( $is_multisite ) : ?>
									<input type="hidden" name="blog_id" value="<?php echo esc_html( $site->blog_id ); ?>">
								<?php endif ?>
								<input type="hidden" name="module_type" value="<?php echo esc_html( $module_type ); ?>">
	...
	<table id="fs_users" class="widefat">
		...
		<?php foreach ( $users as $user_id => $user ) : ?>
			...
			<tr>
				...
				<td><?php echo esc_html( $has_developer_license ? '' : $user->get_name() ); ?></td>
				<td>
					<?php if ( ! $has_developer_license ) : ?>
					<a href="mailto:<?php echo esc_attr( $user->email ); ?>"><?php echo esc_html( $user->email ); ?></a>
					<?php endif ?>
				</td>
				<td><?php echo esc_html( $has_developer_license ? '' : json_encode( $user->is_verified ) ); ?></td>
				<td><?php echo esc_html( $user->public_key ); ?></td>
				<td><?php echo esc_html( $has_developer_license ? FS_Plugin_License::mask_secret_key_for_html( $user->secret_key ) : $user->secret_key ); ?></td>
				<td>
					<?php if ( ! $has_developer_license ) : ?>
					<form action="" method="POST">
						<input type="hidden" name="fs_action" value="delete_user">
						<?php wp_nonce_field( 'delete_user' ); ?>
						<input type="hidden" name="user_id" value="<?php echo esc_html( $user->id ); ?>">
						<button type="submit" class="button"><?php fs_esc_html_echo_x_inline( 'Delete', 'verb', 'delete' ); ?></button>
					</form>
					<?php endif ?>
				</td>
...
<?php foreach ( $module_types as $module_type ) : ?>
	...
	<?php if ( is_array( $licenses ) && count( $licenses ) > 0 ) : ?>
		...
		<table id="fs_<?php echo esc_html( $module_type ); ?>_licenses" class="widefat">
			<thead>
			<tr>
				<th><?php fs_esc_html_echo_inline( 'ID', 'id' ); ?></th>
			...
			<?php foreach ( $licenses as $license ) : ?>
				<tr>
					...
					<td><?php echo esc_html( $license->is_unlimited() ? 'Unlimited' : ( $license->is_single_site() ? 'Single Site' : $license->quota ) ); ?>
					</td>
