...
?>
	<tr class="fs-site-details" data-blog-id="<?php echo esc_attr( $blog_id ); ?>"
														 <?php
															if ( $is_registered ) :
																?>
		 data-install-id="<?php echo esc_attr( $install->id ); ?>"<?php endif ?>>
		<!-- Install ID or Opt-in option -->
		<td><?php if ( $is_registered ) : ?>
				<?php echo esc_attr( $install->id ); ?>
			<?php else : ?>
				<?php $action = 'opt_in'; ?>
		...
		<td><?php if ( $is_registered ) : ?>
				...
				?>
				<code><?php echo esc_attr( $plan_title ); ?></code>
			<?php endif ?></td>
		<!--/ Plan -->
			...
			<table class="widefat fs-key-value-table">
				...
				>
					...
					<td><?php if ( ! FS_Plugin_License::is_valid_id( $install->license_id ) ) : ?>
						...
						<form action="<?php echo esc_url($fs->_get_admin_page_url( 'account' )); ?>" method="POST">
							...
							?>
							" data-is-disconnected="<?php echo esc_attr( $install->is_disconnected ? 'true' : 'false' ); ?>"><?php $install->is_disconnected ? fs_esc_html_echo_inline( 'Opt In', 'opt-in', $slug ) : fs_esc_html_echo_inline( 'Opt Out', 'opt-out', $slug ); ?></button>
						</form>
					<!--/ Toggle Usage Tracking -->
					<?php endif ?></td>
				</tr>
				...
				<?php if ( $is_registered && $install->user_id != $main_user->id ) : ?>
					...
					<?php if ( is_object( $user ) ) : ?>
						...
						>
							...
							<td colspan="2"><code><?php echo esc_url($user->id); ?></code></td>
						</tr>
						<?php $row_index ++; ?>
						<!--/ User ID -->
					<?php endif ?>
				...
				<?php if ( is_object( $license ) ) : ?>
					...
					>
						...
						<td>
							<code><?php echo esc_url($license->get_html_escaped_masked_secret_key()); ?></code>
							<?php if ( ! $is_whitelabeled ) : ?>
						...
						<td>
							...
							<button class="button button-small activate-license-trigger <?php echo esc_url($fs->get_unique_affix()); ?>"><?php fs_esc_html_echo_inline( 'Change License', 'change-license', $slug ); ?></button>
						</td>
						<?php endif ?>
					</tr>
					<?php $row_index ++; ?>
					...
					<?php if ( ! is_object( $main_license ) || $main_license->id != $license->id ) : ?>
						...
						<?php if ( ! $license->is_lifetime() && is_object( $subscription ) ) : ?>
							...
							>
								...
								?>
								<td>
									<code><?php echo esc_html($subscription->id); ?> - 
										<?php
								...
								<td><?php if ( $is_active_subscription ) : ?>
									...
									<form id="fs_downgrade" action="<?php echo esc_url($fs->_get_admin_page_url( 'account' )); ?>" method="POST">
										<input type="hidden" name="fs_action" value="<?php echo esc_attr($action); ?>">
										<?php wp_nonce_field( trim( "{$action}:{$blog_id}", ':' ) ); ?>
										<input type="hidden" name="blog_id" value="<?php echo esc_attr($blog_id); ?>">
										<button class="button button-small" onclick="if (confirm('<?php echo esc_js( $downgrade_confirmation_message . ' ' . $after_downgrade_message . ' ' . $prices_increase_text ); ?>')) { this.parentNode.submit(); } else { return false; }"><?php echo esc_html($downgrade_text); ?></button>
									</form>
								<?php endif ?></td>
							</tr>
							<?php $row_index ++; ?>
