...
if ( $require_license_key ) {
	...
	$optin_params = $fs->get_opt_in_params( [], $is_network_level_activation );
	$sites        = isset( $optin_params['sites'] ) ? $optin_params['sites'] : [];
...
?>
	<div id="fs_connect"
		 ...
		 <?php
			...
			?>
		 <?php echo esc_attr( $require_license_key ? ' require-license-key' : '' ); ?>">
		<div class="fs-visual">
		...
		<div class="fs-content">
			...
			<?php if ( $require_license_key ) : ?>
				<div class="fs-license-key-container">
					<input id="fs_license_key" name="fs_key" type="text" required maxlength="<?php echo esc_js($fs->apply_filters( 'license_key_maxlength', 32 )); ?>"
					placeholder="<?php fs_esc_attr_echo_inline( 'License key', 'license-key', $slug ); ?>" tabindex="1"/>
					<i class="dashicons dashicons-admin-network"></i>
					<a class="show-license-resend-modal show-license-resend-modal-<?php echo esc_js($fs->get_unique_affix()); ?>"
				...
				<div id="fs_marketing_optin">
					...
					<div class="fs-input-container">
						<label>
							...
							<span class="fs-input-label"><?php echo wp_kses_post( $send_updates_text ); ?></span>
						</label>
						<label>
							<input type="radio" name="allow-marketing" value="false" tabindex="1" />
							<span class="fs-input-label"><?php echo wp_kses_post( $do_not_send_updates_text ); ?></span>
						</label>
					</div>
				</div>
			<?php endif ?>
		...
		<div class="fs-actions">
			<?php if ( $fs->is_enable_anonymous() && ! $is_pending_activation && ( ! $require_license_key || $is_network_upgrade_mode ) ) : ?>
				<a id="skip_activation" href="<?php echo fs_nonce_url( $fs->_get_admin_page_url( '', [ 'fs_action' => $fs->get_unique_affix() . '_skip_activation' ], $is_network_level_activation ), $fs->get_unique_affix() . '_skip_activation' ); ?>"
			...
			<?php endif ?>
			<?php if ( $require_license_key ) : ?>
				<a id="license_issues_link" href="<?php echo esc_url($fs->apply_filters( 'known_license_issues_url', 'https://freemius.com/help/documentation/wordpress-sdk/license-activation-issues/' )); ?>" target="_blank"><?php fs_esc_html_echo_inline( 'License issues?', 'license-issues', $slug ); ?></a>
			<?php endif ?>
		</div>
		<?php
		...
		<div class="fs-terms">
			...
			&nbsp;&nbsp;-&nbsp;&nbsp;
			<a href="<?php echo esc_url( $require_license_key ? $freemius_plugin_terms_url : $freemius_usage_tracking_url ); ?>" target="_blank" rel="noopener" tabindex="1"><?php $require_license_key ? fs_echo_inline( 'License Agreement', 'license-agreement', $slug ) : fs_echo_inline( 'Terms of Service', 'tos', $slug ); ?></a>
		</div>
	</div>
	<?php
		/**
...
<script type="text/javascript">
	(function ($) {
		...
		var $primaryCta          = $('.fs-actions .button.button-primary'),
			...
			isNetworkActive      = <?php echo esc_js( $is_network_level_activation ? 'true' : 'false' ); ?>,
			requireLicenseKey    = <?php echo esc_js( $require_license_key ? 'true' : 'false' ); ?>,
			hasContextUser       = <?php echo esc_js( $activate_with_current_user ? 'true' : 'false' ); ?>,
			isNetworkUpgradeMode = <?php echo esc_js( $is_network_upgrade_mode ? 'true' : 'false' ); ?>,
			$licenseSecret,
			...
			if ( ajaxOptin ) {
				if (!hasContextUser || isNetworkUpgradeMode) {
					...
					if ( requireLicenseKey && ! isNetworkDelegating ) {
						...
						action   = '<?php echo esc_js($fs->get_ajax_action( 'network_activate' )); ?>';
						security = '<?php echo esc_js($fs->get_ajax_security( 'network_activate' )); ?>';
					}
					...
					fetchIsMarketingAllowedFlagAndToggleOptin = function () {
						...
						$.ajax({
							...
							data   : {
								...
								security   : '<?php echo esc_js($fs->get_ajax_security( 'fetch_is_marketing_required_flag_value' )); ?>',
								license_key: licenseKey,
								module_id  : '<?php echo esc_js($fs->get_id()); ?>'
							},
							success: function (result) {
