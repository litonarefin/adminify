...
class Freemius extends Freemius_Abstract {
	...
	private function migrate_to_subscriptions_collection() {
		...
		if ( isset( $this->_storage->subscription ) && is_object( $this->_storage->subscription ) ) {
			$this->_storage->subscriptions = [ fs_get_entity( $this->_storage->subscription, FS_Subscription::get_class_name() ) ];
		}
	}
	...
	private function remove_invalid_paths() {
		...
		$file_slug_map = self::$_accounts->get_option( 'file_slug_map', [] );
	...
	private static function migrate_accounts_to_network() {
		...
		foreach ( $sites as $site ) {
			 ...
			 */
			$id_slug_type_path_map = self::$_accounts->get_option( 'id_slug_type_path_map', [], $blog_id );
			...
			foreach ( $blogs_status as $blog_id => $status_or_user_id ) {
				if ( 'skipped' === $status_or_user_id ) {
					...
					if ( ! isset( $opted_in_users[ $status_or_user_id ] ) ) {
						$opted_in_users[ $status_or_user_id ] = [];
					}
				...
				(function ($) {
					$(document).ajaxSend(function (event, jqxhr, settings) {
						if (settings.url &&
							...
							! ( settings.url.indexOf( '<?php echo esc_js($admin_param); ?>' ) > 0 )
						) {
							if (
							...
							settings.url += '<?php echo esc_js($admin_param); ?>=true';
						}
					});
				})(jQuery);
			</script>
	...
	private function register_constructor_hooks() {
		...
		if ( is_admin() ) {
			add_action( 'admin_init', [ &$this, '_hook_action_links_and_register_account_hooks' ] );
			...
			if ( $this->is_plugin() ) {
				if ( self::is_plugin_install_page() && true !== fs_request_get_bool( 'fs_allow_updater_and_dialog' ) ) {
					 ...
					 */
					add_filter( 'site_transient_update_plugins', [ 'Freemius', '_remove_fs_updates_from_plugin_install_page' ], 10, 2 );
					 ...
					 * @since 2.2.3
					 */
					add_action( 'admin_footer', [ 'Freemius', '_prepend_fs_allow_updater_and_dialog_flag_url_param' ] );
				}
				...
				add_action( 'after_switch_theme', [ &$this, '_activate_theme_event_hook' ], 10, 2 );

				add_action( 'admin_footer', [ &$this, '_style_premium_theme' ] );
			}
			...
			if ( ! self::is_ajax() ) {
				if ( ! $this->is_addon() ) {
					add_action( 'init', [ &$this, '_add_default_submenu_items' ], WP_FS__LOWEST_PRIORITY );
				}
			}

			if ( $this->_storage->handle_gdpr_admin_notice ) {
				add_action( 'init', [ &$this, '_maybe_show_gdpr_admin_notice' ] );
			}

			add_action( 'init', [ &$this, '_maybe_add_gdpr_optin_ajax_handler' ] );
			add_action( 'init', [ &$this, '_maybe_add_pricing_ajax_handler' ] );
		}

		if ( $this->is_plugin() ) {
			if ( $this->_is_network_active ) {
				add_action( 'wpmu_new_blog', [ $this, '_after_new_blog_callback' ], 10, 6 );
			}

			register_deactivation_hook( $this->_plugin_main_file_path, [ &$this, '_deactivate_plugin_hook' ] );
		}

		if ( is_multisite() ) {
			add_action( 'deactivate_blog', [ &$this, '_after_site_deactivated_callback' ] );
			add_action( 'archive_blog', [ &$this, '_after_site_deactivated_callback' ] );
			add_action( 'make_spam_blog', [ &$this, '_after_site_deactivated_callback' ] );
			add_action( 'deleted_blog', [ &$this, '_after_site_deleted_callback' ], 10, 2 );

			add_action( 'activate_blog', [ &$this, '_after_site_reactivated_callback' ] );
			add_action( 'unarchive_blog', [ &$this, '_after_site_reactivated_callback' ] );
			add_action( 'make_ham_blog', [ &$this, '_after_site_reactivated_callback' ] );
		}
		...
		if ( $this->is_theme() &&
			...
			add_action( 'customize_register', [ &$this, '_customizer_register' ] );
		}

		add_action( 'admin_init', [ &$this, '_redirect_on_clicked_menu_link' ], WP_FS__LOWEST_PRIORITY );

		if ( $this->is_theme() && ! $this->is_migration() ) {
			add_action( 'admin_init', [ &$this, '_add_tracking_links' ] );
		}

		add_action( 'admin_init', [ &$this, '_add_license_activation' ] );
		add_action( 'admin_init', [ &$this, '_add_premium_version_upgrade_selection' ] );
		add_action( 'admin_init', [ &$this, '_add_beta_mode_update_handler' ] );
		add_action( 'admin_init', [ &$this, '_add_user_change_option' ] );

		$this->add_ajax_action( 'update_billing', [ &$this, '_update_billing_ajax_action' ] );
		$this->add_ajax_action( 'start_trial', [ &$this, '_start_trial_ajax_action' ] );
		$this->add_ajax_action( 'set_data_debug_mode', [ &$this, '_set_data_debug_mode' ] );
		$this->add_ajax_action( 'toggle_whitelabel_mode', [ &$this, '_toggle_whitelabel_mode_ajax_handler' ] );

		if ( $this->_is_network_active && fs_is_network_admin() ) {
			$this->add_ajax_action( 'network_activate', [ &$this, '_network_activate_ajax_action' ] );
		}
		...
		$this->add_ajax_action( 'submit_affiliate_application', [ &$this, '_submit_affiliate_application' ] );

		$this->add_action( 'after_plans_sync', [ &$this, '_check_for_trial_plans' ] );

		$this->add_action( 'sdk_version_update', [ &$this, '_sdk_version_update' ], WP_FS__DEFAULT_PRIORITY, 2 );

		$this->add_action(
			'plugin_version_update',
			[ &$this, '_after_version_update' ],
			WP_FS__DEFAULT_PRIORITY,
			2
		);
		$this->add_filter( 'after_code_type_change', [ &$this, '_after_code_type_change' ] );

		add_action( 'admin_init', [ &$this, '_add_trial_notice' ] );
		add_action( 'admin_init', [ &$this, '_add_affiliate_program_notice' ] );
		add_action( 'admin_enqueue_scripts', [ &$this, '_enqueue_common_css' ] );
		...
		if ( fs_request_is_action( 'reset_anonymous_mode' ) &&
			...
			add_action( 'admin_init', [ &$this, 'connect_again' ] );
		}
	}
	...
	private function register_after_settings_parse_hooks() {
		if ( is_admin() &&
			...
			$this->add_ajax_action(
				...
				[ &$this, '_delete_theme_update_data_action' ]
			);
		}
		...
		if ( $this->show_settings_with_tabs() ) {
			...
			if ( ! $this->_cache->has_valid( 'tabs' ) ) {
				add_action( 'admin_footer', [ &$this, '_tabs_capture' ] );
				// Add license activation AJAX callback.
				$this->add_ajax_action( 'store_tabs', [ &$this, '_store_tabs_ajax_action' ] );

				add_action( 'admin_enqueue_scripts', [ &$this, '_store_tabs_styles' ], 9999999 );
			}

			add_action(
				'admin_footer',
				[ &$this, '_add_freemius_tabs' ],
				/**
	...
	static function _maybe_add_beta_label_to_plugins_and_handle_confirmation() {
		...
		foreach ( self::$_instances as $instance ) {
			...
			$beta_data[ $instance->get_plugin_basename() ] = [ 'is_installed_version_beta' => $is_beta ];
	...
	private function _register_account_hooks() {
		...
		$this->add_ajax_action(
			...
			[ &$this, '_submit_uninstall_reason_action' ]
		);

		$this->add_ajax_action(
			'cancel_subscription_or_trial',
			[ &$this, 'cancel_subscription_or_trial_ajax_action' ]
		);
		...
		if ( ! $this->is_addon() || $this->is_parent_plugin_installed() ) {
			if ( ( $this->is_plugin() && self::is_plugins_page() ) ||
				...
				add_action( 'admin_footer', [ &$this, '_add_deactivation_feedback_dialog_box' ] );
			}
		}
	}
	...
	private function _find_caller_plugin_file( $is_init = false ) {
		...
		// Find caller module.
		$id_slug_type_path_map            = self::$_accounts->get_option( 'id_slug_type_path_map', [] );
		$this->_storage->plugin_main_file = (object) [
	...
	private function store_id_slug_type_path_map( $module_id, $slug ) {
		...
		if ( ! isset( $id_slug_type_path_map[ $module_id ] ) ) {
			$id_slug_type_path_map[ $module_id ] = [
				'slug' => $slug,
			];
	...
	function _get_uninstall_reasons( $user_type = 'long-term' ) {
		...
		$uninstall_reasons = [
			...
			'non-registered-and-non-anonymous-short-term' => [
				[
	...
	private static function _load_required_static() {
		...
		if ( ! WP_FS__DEMO_MODE ) {
			add_action(
				( fs_is_network_admin() ? 'network_' : '' ) . 'admin_menu',
				[
					'Freemius',
					'_add_debug_section',
				]
		...
		add_action( 'wp_ajax_fs_toggle_debug_mode', [ 'Freemius', '_toggle_debug_mode' ] );

		self::add_ajax_action_static( 'get_debug_log', [ 'Freemius', '_get_debug_log' ] );

		self::add_ajax_action_static( 'get_db_option', [ 'Freemius', '_get_db_option' ] );

		self::add_ajax_action_static( 'set_db_option', [ 'Freemius', '_set_db_option' ] );

		if ( 0 == did_action( 'plugins_loaded' ) ) {
			add_action( 'plugins_loaded', [ 'Freemius', '_load_textdomain' ], 1 );
		}

		add_action( 'admin_footer', [ 'Freemius', '_enrich_ajax_url' ] );
		add_action( 'admin_footer', [ 'Freemius', '_open_support_forum_in_new_page' ] );

		if ( self::is_plugins_page() || self::is_themes_page() ) {
			add_action( 'admin_print_footer_scripts', [ 'Freemius', '_maybe_add_beta_label_styles' ], 9 );
			 ...
			 * @since 2.3.0
			 */
			add_action( 'admin_footer-' . self::get_current_page(), [ 'Freemius', '_maybe_add_beta_label_to_plugins_and_handle_confirmation' ] );
		}
	...
	static function _add_debug_section() {
		...
		if ( ! empty( $hook ) ) {
			add_action( "load-$hook", [ 'Freemius', '_debug_page_actions' ] );
		}
	}
	...
	static function _toggle_debug_mode() {
		...
		if ( fs_request_is_post() && in_array( $is_on, [ 0, 1 ] ) ) {
	...
	private function store_connectivity_info( $pong, $is_connected ) {
		...
		$this->_storage->connectivity_test = [
	...
	function _email_about_firewall_issue() {
		...
		if ( $is_connected ) {
			...
			echo esc_url($this->get_after_plugin_activation_redirect_url());
			exit;
		}
		...
		$custom_email_sections['api_error'] = [
			'title' => 'API Error',
			'rows'  => [
				'ping' => [
					'API Error',
		...
		$this->send_email(
			...
			$custom_email_sections,
			[ "Reply-To: $admin_email <$admin_email>" ] // headers
		);
	...
	private function send_email(
		...
		$vars    = [ 'sections' => $default_sections ];
	...
	private function get_email_sections() {
		...
		$sections = [
			'sdk'      => [
				'title' => 'SDK',
				'rows'  => [
					'fs_version'   => [ 'FS Version', $this->version ],
					'curl_version' => [ 'cURL Version', $curl_version_information['version'] ],
				],
			],
			'plugin'   => [
				'title' => ucfirst( $this->get_module_type() ),
				'rows'  => [
					'name'    => [ 'Name', $this->get_plugin_name() ],
			...
			'site'     => [
				'title' => 'Site',
				'rows'  => [
					'unique_id'   => [ 'Unique ID', $this->get_anonymous_id() ],
					'address'     => [ 'Address', site_url() ],
					'host'        => [
						'HTTP_HOST',
			...
			'user'     => [
				...
				'rows'  => [
					...
					'first' => [ 'First', $current_user->user_firstname ],
					'last'  => [ 'Last', $current_user->user_lastname ],
				],
			],
			'plugins'  => [
				'title' => 'Plugins',
				'rows'  => [
					'active_plugins' => [ 'Active Plugins', $active_plugin_string ],
				],
			],
			'php_info' => [
				'title' => 'PHP Info',
	...
	function dynamic_init( array $plugin_info ) {
				...
				if ( ! $this->has_api_connectivity() ) {
					if ( $this->_admin_notices->has_sticky( 'failed_connect_api_first' ) ||
						...
						if ( ! $this->_enable_anonymous || $this->is_premium() ) {
							...
							add_action( 'admin_footer', [ 'Freemius', '_add_firewall_issues_javascript' ] );

							$ajax_action_suffix = $this->_slug . ( $this->is_theme() ? ':theme' : '' );
							add_action(
								"wp_ajax_fs_resolve_firewall_issues_{$ajax_action_suffix}",
								[
									&$this,
							...
							add_action(
								"wp_ajax_fs_retry_connectivity_test_{$ajax_action_suffix}",
								[
									&$this,
		...
		if ( $this->is_user_in_admin() ) {
			...
			add_action( 'admin_init', [ &$this, '_admin_init_action' ] );
	...
	function stop_site_tracking() {
		...
		// Send update to FS.
		$result = $this->get_api_site_scope()->call(
			'/?fields=is_disconnected',
			'put',
			[
	...
	private function parse_settings( &$plugin_info ) {
		...
		$this->_permissions                               = $this->get_option( $plugin_info, 'permissions', [] );
		...
		if ( $this->is_registered() ) {
			if ( ! $this->is_addon() ) {
				add_action(
					...
					[ &$this, '_plugin_code_type_changed' ]
				);
			}
	...
	private function deactivate_premium_only_addon_without_license( $is_after_trial_cancel = false ) {
		if ( ! $this->has_free_plan() &&
			...
			if ( ! $this->has_free_plan() &&
				...
				deactivate_plugins( [ $this->_plugin_basename ], true );
	...
	private function execute_cron( $name, $callable ) {
		...
		foreach ( $users_2_blog_ids as $user_id => $blog_ids ) {
			...
			call_user_func_array( $callable, [ $blog_ids, ( is_multisite() ? $current_blog_id : null ) ] );
	...
	function _sync_cron() {
		...
		$this->execute_cron( 'sync', [ &$this, '_sync_cron_method' ] );
	}
	...
	private function hook_callback_to_sync_cron() {
		$this->add_action( 'data_sync', [ &$this, '_sync_cron' ] );
	}
	...
	private function hook_callback_to_install_sync() {
		$this->add_action( 'install_sync', [ &$this, '_run_sync_install' ] );
	}
	...
	public function _run_sync_install() {
		...
		$this->execute_cron( 'sync', [ &$this, '_sync_install_cron_method' ] );
	}
	...
	function _sync_install_cron_method( array $blog_ids, $current_blog_id = null ) {
		if ( $this->is_registered() ) {
			if ( 1 < count( $blog_ids ) ) {
				$this->sync_installs( [], true );
			} else {
				$this->sync_install( [], true );
			}
						...
						if ( ! $this->_anonymous_mode &&
							...
							$this->_admin_notices->add(
								sprintf(
									...
									sprintf(
										'<b><a href="%s">%s</a></b>',
										$this->get_activation_url( [], ! $this->is_delegated_connection() ),
										sprintf(
											$this->get_text_x_inline(
			...
			<script type="text/javascript">// <![CDATA[
				jQuery(document).ready(function ($) {
					if ('undefined' !== typeof(jQuery().pointer)) {
						...
						var element = <?php echo esc_js($this->apply_filters( 'optin_pointer_element', '$("#non_existing_element");' )); ?>;
						...
						<?php
						echo esc_js($this->apply_filters(
	...
	private function maybe_activate_addon_license() {
		...
		if ( ! $this->is_registered() ) {
			...
			$install = $this->get_api_site_scope()->call(
				'/',
				'put',
				[ 'license_key' => $this->apply_filters( 'license_key', $license->secret_key ) ]
			);
	 ...
	 * @param int               $blog_id
	 */
	private function maybe_activate_bundle_license( FS_Plugin_License $license = null, $sites = [], $blog_id = 0 ) {
	 ...
	 */
	private function activate_bundle_license( $license, $sites = [], $current_blog_id = 0 ) {
		$is_network_admin = fs_is_network_admin();

		$installs_by_blog_map  = [];
		...
		foreach ( $license->products as $product_id ) {
			...
			if ( ! $is_network_admin || $current_blog_id > 0 ) {
				...
				if ( empty( $sites ) ) {
					...
					foreach ( $all_sites as $site ) {
						$sites[] = [ 'blog_id' => self::get_site_blog_id( $site ) ];
					}
				} else {
	...
	function delete_network_account_event( $check_user = true ) {
		...
		foreach ( $sites as $site ) {
			...
			// Clear all storage data.
			$this->_storage->clear_all( true, [ 'connectivity_test' ], $blog_id );
	...
	function _deactivate_plugin_hook() {
		...
		register_uninstall_hook( $this->_plugin_main_file_path, [ 'Freemius', '_uninstall_plugin_hook' ] );
	...
	function skip_connection( $sites = null, $skip_all_network = false ) {
		...
		if ( ! $skip_all_network && empty( $sites ) ) {
			...
			$this->get_api_plugin_scope()->call(
				'skip.json',
				'put',
				[
					'uids' => $uids,
	...
	private function get_plugins_data_for_api() {
		...
		if ( ! is_object( $site_active_plugins_cache ) ) {
			$site_active_plugins_cache = (object) [
				'timestamp' => '',
				'md5'       => '',
				'plugins'   => [],
			];
	 ...
	 * @return array
	 */
	private function get_install_diff_for_api( $site, $install, $override = [] ) {
		$diff             = [];
		$special          = [];
	 ...
	 */
	private function send_install_update( $override = [], $flush = false ) {
		$this->_logger->entrance();
	 ...
	 * @return false|object|string
	 */
	private function send_installs_update( $override = [], $flush = false ) {
		$this->_logger->entrance();
	 ...
	 * @param bool            $flush
	 */
	private function sync_install( $override = [], $flush = false ) {
		$this->_logger->entrance();
	 ...
	 * @param bool            $flush
	 */
	private function sync_installs( $override = [], $flush = false ) {
		$this->_logger->entrance();
	 ...
	 */
	public function track_event( $name, $properties = [], $process_at = false, $once = false ) {
		$this->_logger->entrance(
			http_build_query(
				[
					'name' => $name,
	 ...
	 */
	public function track_event_once( $name, $properties = [], $process_at = false ) {
	...
	private static function find_slug_by_basename( $plugin_base_name ) {
		...
		if ( ! [ $file_slug_map ] || ! isset( $file_slug_map[ $plugin_base_name ] ) ) {
			return false;
		}
	...
	static function get_all_users() {
		$users = self::maybe_get_entities_account_option( 'users', [] );
	...
	private static function get_account_option( $option_name, $module_type = null, $network_level_or_blog_id = null ) {
		...
		return self::maybe_get_entities_account_option( $option_name, [], $network_level_or_blog_id );
	}
	...
	private static function get_user_id_license_ids_map( $module_id, $user_id = null ) {
		...
		if ( ! is_array( $all_modules_user_id_license_ids_map ) ) {
			$all_modules_user_id_license_ids_map = [];
		}
	...
	private static function get_all_updates() {
		$updates = self::maybe_get_entities_account_option( 'updates', [] );
	...
	private static function get_all_addons() {
		$addons = self::maybe_get_entities_account_option( 'addons', [] );
	...
	private static function get_all_account_addons() {
		$addons = self::$_accounts->get_option( 'account_addons', [] );
	...
	function _get_addon_info( $addon_id, $is_installed ) {
		...
		if ( ! fs_is_network_admin() ) {
			...
			$sites = self::maybe_get_entities_account_option( 'sites', [] );
		} else {
			$sites = null;
		...
		$plugins_data = self::maybe_get_entities_account_option( WP_FS__MODULE_TYPE_PLUGIN . 's', [] );
		...
		$all_plans = self::maybe_get_entities_account_option( 'plans', [] );
		...
		$licenses = self::maybe_get_entities_account_option( 'all_licenses', [] );
	...
	private function can_activate_license_on_network( FS_Plugin_License $license ) {
		...
		$installs_without_license = [];
		$disconnected_site_ids    = [];
	...
	private function activate_license_on_many_sites(
		...
		foreach ( $site_ids as $site_id ) {
			$sites[] = $this->get_site_info( [ 'blog_id' => $site_id ] );
		}
	...
	function store_subscription( FS_Subscription $subscription ) {
		if ( ! isset( $this->_storage->subscriptions ) ) {
			$this->_storage->subscriptions = [];
		}

		if ( empty( $this->_storage->subscriptions ) || ! is_multisite() ) {
			$this->_storage->subscriptions = [ $subscription ];
	...
	function delete_unused_subscriptions() {
		...
		$subscriptions_to_keep_by_license_id_map = [];
		$sites                                   = self::get_sites();
		foreach ( $sites as $site ) {
			$blog_id = self::get_site_blog_id( $site );
			$install = $this->get_install_by_blog_id( $blog_id );
	...
	function _add_license_activation() {
		...
		// Add license activation AJAX callback.
		$this->add_ajax_action( 'activate_license', [ &$this, '_activate_license_ajax_action' ] );

		// Add resend license AJAX callback.
		$this->add_ajax_action( 'resend_license_key', [ &$this, '_resend_license_key_ajax_action' ] );
	}
	...
	function _add_user_change_option() {
		...
		$this->add_ajax_action( 'change_user', [ &$this, '_user_change_ajax_action' ] );
	}
	...
	function _toggle_whitelabel_mode_ajax_handler() {
		...
		$license = $this->get_api_user_scope()->call(
			...
			'put',
			[ 'is_whitelabeled' => ! $this->_license->is_whitelabeled ]
		);
		...
		self::shoot_ajax_response( [ 'success' => true ] );
	}
	...
	function _add_beta_mode_update_handler() {
		...
		$this->add_ajax_action( 'set_beta_mode', [ &$this, '_set_beta_mode_ajax_handler' ] );
	}
	...
	function _set_beta_mode_ajax_handler() {
		...
		if ( empty( $is_beta ) || ! in_array( $is_beta, [ 'true', 'false' ] ) ) {
			self::shoot_ajax_failure();
		}
	...
	private function get_parent_and_addons_installs_info() {
		...
		foreach ( $addons_ids as $addon_id ) {
			...
			$installs_info_by_slug_map[ $addon_info['slug'] ] = [
	...
	function _network_activate_ajax_action() {
		...
		$sites = fs_request_get( 'sites', [], 'post' );
	...
	function _resend_license_key_ajax_action() {
		...
		if ( is_object( $result ) && isset( $result->error ) ) {
			...
			if ( in_array( $error->code, [ 'invalid_email', 'no_user' ] ) ) {
	 ...
	 * @return string
	 */
	function _get_admin_page_url( $page = '', $params = [], $network = null ) {
		if ( is_null( $network ) ) {
			$network = (
				$this->_is_network_active &&
	...
	function is_active_for_site( $blog_id ) {
		...
		return in_array( $this->_plugin_basename, (array) get_blog_option( $blog_id, 'active_plugins', [] ) );
	}
	 ...
	 * @return string
	 */
	function _get_sync_license_url( $plugin_id = false, $add_action_nonce = true, $params = [] ) {
	 ...
	 * @return string
	 */
	function get_account_url( $action = false, $params = [], $add_action_nonce = true ) {
		if ( is_string( $action ) ) {
	 ...
	 */
	function get_account_tab_url( $tab, $action = false, $params = [], $add_action_nonce = true ) {
		$params['tab'] = $tab;
	...
	private function sync_user_by_current_install( $site_user_id = null ) {
		...
		if ( in_array( $error_code, [ 'invalid_unique_id', 'user_cannot_be_recovered' ] ) ) {
			/**
	 ...
	 * @return array
	 */
	function get_opt_in_params( $override_with = [], $network_level_or_blog_id = null ) {
		$this->_logger->entrance();
		...
		$return_url        = $this->is_anonymous() ?
			...
			$this->get_account_url( $activation_action, [], false ) :
			// Return to the module's main page.
			$this->get_after_activation_url( 'after_connect_url', [ 'fs_action' => $activation_action ] );
		...
		if ( true === $network_level_or_blog_id ) {
			...
			$site = is_numeric( $network_level_or_blog_id ) ?
				[ 'blog_id' => $network_level_or_blog_id ] :
				null;
	...
	function opt_in(
		...
		if ( ! $this->is_api_result_object( $decoded ) ) {
			...
			if ( $is_network ) {
				 ...
				 * @author Leo Fajardo (@leorw)
				 */
				$this->_storage->pending_sites_info = [
					'blog_ids'      => $site_ids,
	...
	function setup_network_account(
		...
		if ( is_numeric( $plugin_id ) ) {
			...
			$next_page = $this->is_anonymous() ?
				...
				$this->get_after_activation_url( 'after_connect_url', [], $is_network_level_opt_in );
		}
	...
	private function create_installs_with_user(
		...
		if ( ! $this->is_api_result_entity( $result ) &&
			...
			if ( $redirect ) {
				...
				fs_redirect( $this->get_activation_url( [ 'error' => $result->error->message ] ) );
			}
		}
		...
		if ( $this->is_activation_mode() ) {
			if ( $this->show_opt_in_on_setting_page() ) {
				...
				if ( fs_request_is_action( $this->get_unique_affix() . '_activate_existing' ) ) {
					add_action( 'load-themes.php', [ &$this, '_install_with_current_user' ] );
				} elseif ( fs_request_is_action( $this->get_unique_affix() . '_activate_new' ) ||
							fs_request_get_bool( 'pending_activation' )
				) {
					add_action( 'load-themes.php', [ &$this, '_install_with_new_user' ] );
				}
			}
		} else {
			if ( ! $this->is_registered() ) {
	...
	private function override_plugin_menu_with_activation() {
		...
		if ( ! $this->has_settings_menu() ) {
			...
			$hook = $this->_menu->override_menu_item( [ &$this, '_connect_page_render' ] );
			...
			$menus = [ $this->_menu->get_parent_slug() ];
	 ...
	 */
	private function _store_licenses( $store = true, $module_id = false, $licenses = [] ) {
		$this->_logger->entrance();
	...
	private function _sync_plugin_license(
		...
		if ( ! $send_installs_update ) {
			...
			if ( $is_site_level_sync ) {
				...
				$result   = $this->send_install_update( [], true );
				$is_valid = $this->is_api_result_entity( $result );
			} else {
				$result   = $this->send_installs_update( [], true );
	...
	public function _open_license_activation_dialog_box() {
		$vars = [ 'license_id' => $this->_site->license_id ];
	...
	protected function _activate_license( $background = false, $premium_license = null ) {
		...
		if ( $this->_site->user_id != $premium_license->user_id ) {
			$api_request_params = [ 'license_key' => $premium_license->secret_key ];
		} else {
			$api_request_params = [];
		}
	...
	private function _downgrade_site() {
		...
		$api  = $this->get_api_site_scope();
		$site = $api->call( 'downgrade.json', 'put', [ 'deactivate_license' => $deactivate_license ] );
	 ...
	 * @return string
	 */
	function get_activation_url( $params = [], $network = null ) {
	 ...
	 * @return string
	 */
	function get_reconnect_url( $params = [] ) {
	 ...
	 * @return string
	 */
	function get_after_activation_url( $filter, $params = [], $network = null ) {
		if ( $this->show_opt_in_on_themes_page() &&
		...
		if ( $this->_is_network_active &&
			...
			$target_url = $this->_get_admin_page_url( '', [], $network );
		}
	...
	private function _handle_account_edits() {
			...
			case 'activate_license':
				...
				if ( is_object( $fs ) ) {
					...
					if ( $this->is_bundle_license_auto_activation_enabled() ) {
						$fs->maybe_activate_bundle_license( null, [], is_numeric( $blog_id ) ? $blog_id : 0 );
					}
				}
		...
		if ( WP_FS__IS_POST_REQUEST ) {
			$properties = [ 'site_secret_key', 'site_id', 'site_public_key' ];
	...
	function _affiliation_page_render() {
		...
		$vars = [ 'id' => $this->_module_id ];
		print $this->apply_filters( '/forms/affiliation.php', fs_get_template( '/forms/affiliation.php', $vars ) );
	}
	...
	function _account_page_render() {
		 ...
		 * @since  1.2.1.6
		 */
		print $this->apply_filters( "templates/{$template}", fs_get_template( $template, $vars ) );
	}
	...
	function _connect_page_render() {
		 ...
		 * @since  1.2.1.6
		 */
		print $this->apply_filters( 'templates/connect.php', fs_get_template( 'connect.php', $vars ) );
	}
	...
	function _addons_page_render() {
		 ...
		 * @since  1.2.1.6
		 */
		$this->apply_filters( 'templates/add-ons.php', fs_get_template( 'add-ons.php', $vars ) );
	}
	...
	function _pricing_page_render() {
		...
		if ( 'true' === fs_request_get( 'checkout', false ) ) {
			print $this->apply_filters( 'templates/checkout.php', fs_get_template( 'checkout.php', $vars ) );
		} else {
			$this->apply_filters( 'templates/pricing.php', fs_get_template( 'pricing.php', $vars ) );
		}
	}
	...
	function _maybe_add_pricing_ajax_handler() {
		if ( ! $this->should_use_external_pricing() ) {
			$this->add_ajax_action( 'pricing_ajax_action', [ &$this, '_fs_pricing_ajax_action_handler' ] );
		}
	}
	...
	function _contact_page_render() {
		 ...
		 * @since  2.1.3
		 */
		print $this->apply_filters( 'templates/contact.php', fs_get_template( 'contact.php', $vars ));
	}
	...
	function _fix_start_trial_menu_item_url() {
		$template_args = [ 'id' => $this->_module_id ];
	...
	function _add_trial_notice() {
		...
		if ( $this->is_in_trial_promotion() ) {
			add_action( 'admin_footer', [ &$this, '_fix_start_trial_menu_item_url' ] );
	...
	function _add_affiliate_program_notice() {
		...
		$message = sprintf(
			__( 'Hey there, did you know that <strong>%1$s</strong> has an affiliate program? If you like the %2$s you can become our ambassador and earn some cash!', 'freemius' ),
			$this->get_plugin_name(),
			$this->get_module_label( true )
		);
	...
	function _show_theme_activation_optin_dialog() {
		...
		add_action( 'admin_footer', [ &$this, '_add_fs_theme_activation_dialog' ] );
	}
	...
	function _add_license_action_link() {
		...
		if ( ! self::is_ajax() ) {
			...
			add_action( 'admin_footer', [ &$this, '_add_license_activation_dialog_box' ] );
		}
	...
	function _add_premium_version_upgrade_selection_action() {
		...
		if ( ! self::is_ajax() ) {
			add_action( 'admin_footer', [ &$this, '_add_premium_version_upgrade_selection_dialog_box' ] );
		}
	}
	...
	function _add_tracking_links() {
		...
		if ( $this->add_ajax_action( 'stop_tracking', [ &$this, '_stop_tracking_callback' ] ) ) {
			return;
		}

		if ( $this->add_ajax_action( 'allow_tracking', [ &$this, '_allow_tracking_callback' ] ) ) {
			return;
		}

		if ( $this->add_ajax_action( 'update_tracking_permission', [ &$this, '_update_tracking_permission_callback' ] ) ) {
			return;
		}
		...
		add_action( 'admin_footer', [ &$this, '_add_optout_dialog' ] );
	...
	private function get_license_network_activation_notice() {
		...
		return sprintf(
			...
			'<a href="' . $this->get_account_url( false, [ 'activate_license' => 'true' ] ) . '">',
			'</a>'
		);
	}
	...
	function _add_tabs_before_content() {
		 ...
		 */
		$stylesheets = $this->_cache->get( 'tabs_stylesheets', [] );
		...
		if ( is_string( $local_path ) ) {
			...
			if ( ! is_array( $icons ) || 0 === count( $icons ) ) {
				if ( ! WP_FS__IS_LOCALHOST && $this->is_theme() ) {
					...
					$have_write_permissions = ( 'direct' === get_filesystem_method( [], fs_normalize_path( $img_dir ) ) );
	...
	function _maybe_show_gdpr_admin_notice() {
		...
		$modules = array_merge(
			array_values( self::maybe_get_entities_account_option( 'plugins', [] ) ),
			array_values( self::maybe_get_entities_account_option( 'themes', [] ) )
		);
	...
	function _maybe_add_gdpr_optin_ajax_handler() {
		$this->add_ajax_action( 'fetch_is_marketing_required_flag_value', [ &$this, '_fetch_is_marketing_required_flag_value_ajax_action' ] );
	...
	private function add_gdpr_optin_ajax_handler_and_style() {
		...
		$this->add_ajax_action( 'gdpr_optin_action', [ &$this, '_gdpr_optin_ajax_action' ] );

		add_action( 'admin_footer', [ &$this, '_add_gdpr_optin_js' ] );
		add_action( 'admin_enqueue_scripts', [ &$this, 'enqueue_gdpr_optin_notice_style' ] );
	}
	...
	function _gdpr_optin_ajax_action() {
		...
		$plugin_ids = fs_request_get( 'plugin_ids', [] );
