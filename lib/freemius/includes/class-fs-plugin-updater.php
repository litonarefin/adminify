...
class FS_Plugin_Updater {
	...
	private function filters() {
		...
		if ( ! $this->_fs->has_any_active_valid_license() ) {
			add_action( 'admin_head', [ &$this, 'catch_plugin_information_dialog_contents' ] );
		}
		...
		if ( $this->_fs->is_premium() ) {
			if ( ! $this->is_correct_folder_name() ) {
				add_filter( 'upgrader_post_install', [ &$this, '_maybe_update_folder_name' ], 10, 3 );
			}

			add_filter( 'upgrader_pre_install', [ 'FS_Plugin_Updater', '_store_basename_for_source_adjustment' ], 1, 2 );
			add_filter( 'upgrader_source_selection', [ 'FS_Plugin_Updater', '_maybe_adjust_source_dir' ], 1, 3 );

			if ( ! $this->_fs->has_any_active_valid_license() ) {
				add_filter( 'wp_prepare_themes_for_js', [ &$this, 'change_theme_update_info_html' ], 10, 1 );
			}
		}
	}
	...
	function catch_plugin_information_dialog_contents() {
		...
		add_action( 'admin_footer', [ &$this, 'edit_and_echo_plugin_information_dialog_contents' ], 0, 1 );
	...
	function edit_and_echo_plugin_information_dialog_contents( $hook_suffix ) {
		...
		echo Utils::wp_kses_custom($contents);
	}
	...
	function edit_and_echo_plugin_update_row( $file, $plugin_data ) {
		...
		if ( ! isset( $current->response[ $file ] ) ) {
			echo Utils::wp_kses_custom($plugin_update_row);
	...
	function pre_set_site_transient_update_plugins_filter( $transient_data ) {
		...
		if ( $this->_fs->is_org_repo_compliant() && $this->_fs->is_freemium() ) {
			...
			if ( ! empty( $this->_translation_updates ) ) {
				...
				$current_plugin_translation_updates_map = [];
		...
		if ( ! is_object( $new_version ) || empty( $new_version->version ) ) {
			...
			if ( isset( $new_version->readme ) && is_object( $new_version->readme ) ) {
				...
				if ( isset( $new_version_readme_data->sections ) ) {
					...
					$new_version_readme_data->sections = [];
				}
