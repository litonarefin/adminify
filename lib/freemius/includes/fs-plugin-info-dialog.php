...
class FS_Plugin_Info_Dialog {
	...
	function __construct( Freemius $fs ) {
		...
		add_action( 'install_plugins_pre_plugin-information', [ &$this, 'install_plugin_information' ] );

		// Override request for plugin information for Add-ons.
		add_filter(
			'fs_plugins_api',
			[ &$this, '_get_addon_info_filter' ],
	...
	function _get_addon_info_filter( $data, $action = '', $args = null ) {
		...
		$view_vars  = [ 'plugin' => $selected_addon ];
	...
	private static function get_blog_status_url( $blog_id, $network_status_url, $status ) {
		if ( ! in_array( $status, [ 'install', 'update_available' ] ) ) {
	...
	function install_plugin_information() {
		...
		<div id="<?php echo esc_attr($_tab); ?>-content" class='<?php echo esc_attr($_with_banner); ?>'>
						...
						<?php foreach ( $api->plans as $plan ) : ?>
							...
							" data-plan-id="<?php echo esc_attr($plan->id); ?>">
								...
								<div class="nav-tab-wrapper">
									<?php $billing_cycles = [ 'monthly', 'annual', 'lifetime' ]; ?>
									<?php
									$i = 0;
										...
										(function ($, undef) {
											var
												...
												_checkoutUrl            = function (plan, pricing, cycle) {
													return '<?php echo esc_url_raw( remove_query_arg( 'billing_cycle', add_query_arg( [ 'plugin_id' => $plan->plugin_id ], $api->checkout_link ) ) ); ?>' +
														'&plan_id=' + plan +
														'&pricing_id=' + pricing +
														'&billing_cycle=' + cycle
														<?php
												...
												$plan.find('.nav-tab').click(function () {
													...
													if (1 == pricing.length) {
														...
														for (var i = 0; i < pricing.length; i++) {
															html += '<li><label><input name="pricing-<?php echo esc_js($plan->id); ?>" type="radio" value="' + pricing[i].id + '">' + _formatPrice(pricing[i], billingCycle) + '</label></li>';
														}
													}
								...
								<div class="fs-pricing-body">
									...
									<?php echo Utils::wp_kses_custom($this->get_actions_dropdown( $api, $plan )); ?>
						...
						<?php if ( ! empty( $api->version ) ) { ?>
							<li>
								<strong><?php fs_esc_html_echo_x_inline( 'Version', 'product version', 'version', $api->slug ); ?>
									:</strong> <?php echo Utils::wp_kses_custom($api->version); ?></li>
							<?php
						}
						if ( ! empty( $api->author ) ) {
							?>
						...
						if ( ! empty( $api->last_updated ) ) {
							?>
								<li><strong><?php fs_echo_inline( 'Last Updated', 'last-updated', $api->slug ); ?>
										:</strong> <span
										title="<?php echo esc_attr($api->last_updated); ?>">
							<?php
							echo esc_html(
								sprintf(
							...
							?>
								<li>
									<strong><?php fs_esc_html_echo_inline( 'Compatible up to', 'compatible-up-to', $api->slug ); ?>
										:</strong> <?php echo Utils::wp_kses_custom($api->tested); ?>
								</li>
								<?php
						}
						...
						if ( ! empty( $api->slug ) && true == $api->is_wp_org_compliant ) {
							?>
								<li><a target="_blank"
									   ...
									   href="https://wordpress.org/plugins/<?php echo esc_url( $api->slug ); ?>/"><?php fs_esc_html_echo_inline( 'WordPress.org Plugin Page', 'wp-org-plugin-page', $api->slug ); ?>
										&#187;</a>
								</li>
								<?php
						}
			...
			foreach ( (array) $api->sections as $section_name => $content ) {
				...
				echo Utils::wp_kses_custom($content);
				echo "\t</div>\n";
			}
			echo "</div>\n";
			echo "</div>\n";
			...
			! empty( $api->download_link ) &&
			! empty( $this->status ) &&
			in_array( $this->status['status'], [ 'newer_installed', 'latest_installed' ] )
			) {
				if ( 'newer_installed' === $this->status['status'] ) {
					echo Utils::wp_kses_custom($this->get_cta(
			...
			echo Utils::wp_kses_custom($this->get_actions_dropdown( $api, null ));
