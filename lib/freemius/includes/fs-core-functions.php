...
if ( ! function_exists( 'fs_enqueue_local_style' ) ) {
	function fs_enqueue_local_style( $handle, $path, $deps = [], $ver = false, $media = 'all' ) {
		wp_enqueue_style( $handle, fs_asset_url( WP_FS__DIR_CSS . '/' . trim( $path, '/' ) ), $deps, $ver, $media );
	}
}

if ( ! function_exists( 'fs_enqueue_local_script' ) ) {
	function fs_enqueue_local_script( $handle, $path, $deps = [], $ver = false, $in_footer = 'all' ) {
...
if ( ! function_exists( 'fs_ui_get_action_button' ) ) {
	...
	function fs_ui_get_action_button(
		...
		if ( is_string( $confirmation ) ) {
			...
			return sprintf(
				...
				wp_nonce_url( freemius( $module_id )->_get_admin_page_url( $page, array_merge( $params, [ 'fs_action' => $action ] ) ), $action ),
				'button' . ( ! empty( $button_class ) ? ' ' . $button_class : '' ) . ( $is_primary ? ' button-primary' : '' ) . ( $is_small ? ' button-small' : '' ),
				$title
			);
		}
	...
	function fs_ui_action_link( $module_id, $page, $action, $title, $params = [] ) {
		?><a class="" href="<?php echo wp_nonce_url( freemius( $module_id )->_get_admin_page_url( $page, array_merge( $params, [ 'fs_action' => $action ] ) ), $action ); ?>"><?php echo esc_html($title); ?></a>
								  <?php
	}
}
...
if ( ! function_exists( 'fs_canonize_url' ) ) {
	 ...
	 * @return string
	 */
	function fs_canonize_url( $url, $omit_host = false, $ignore_params = [] ) {
...
if ( ! function_exists( 'fs_download_image' ) ) {
	...
	function fs_download_image( $from, $to ) {
		...
		if ( 'direct' !== get_filesystem_method( [], $dir ) ) {
			return false;
		}
