...
class FS_Api {
	 ...
	 */
	private function _call( $path, $method = 'GET', $params = [], $retry = false ) {
	 ...
	 * @return bool
	 */
	function is_cached( $path, $method = 'GET', $params = [] ) {
	 ...
	 * @param array  $params
	 */
	function purge_cache( $path, $method = 'GET', $params = [] ) {
		$this->_logger->entrance( "{$method}:{$path}" );
	 ...
	 * @param array  $params
	 */
	function update_cache_expiration( $path, $expiration = WP_FS__TIME_24_HOURS_IN_SEC, $method = 'GET', $params = [] ) {
	 ...
	 * @throws \Freemius_Exception
	 */
	private function get_cache_key( $path, $method = 'GET', $params = [] ) {
	 ...
	 * @return object
	 */
	function ping( $unique_anonymous_id = null, $params = [] ) {
		$this->_logger->entrance();
