...
class FS_Admin_Notice_Manager {
	...
	protected function __construct(
		...
		if ( ( $this->_is_network_notices && $is_network_admin ) ||
			...
			if ( 0 < count( $this->_sticky_storage ) ) {
				...
				add_action(
					"wp_ajax_fs_dismiss_notice_action_{$ajax_action_suffix}",
					[
						&$this,
	...
	private static function has_sticky_messages() {
		if ( ! self::$_added_sticky_javascript ) {
			add_action( 'admin_footer', [ 'FS_Admin_Notice_Manager', '_add_sticky_dismiss_javascript' ] );
		}
	}
	...
	function add(
		...
		if ( empty( $this->_notices ) ) {
			if ( ! $is_network_and_blog_admins ) {
				add_action( $notices_type, [ &$this, '_admin_notices_hook' ] );
			} else {
				add_action( 'network_admin_notices', [ &$this, '_admin_notices_hook' ] );
				add_action( 'admin_notices', [ &$this, '_admin_notices_hook' ] );
			}

			add_action( 'admin_enqueue_scripts', [ &$this, '_enqueue_styles' ] );
		}
