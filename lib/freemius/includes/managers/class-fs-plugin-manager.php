...
class FS_Plugin_Manager {
	...
	protected function get_all_modules( $module_type = false ) {
		...
		if ( false !== $module_type ) {
			return fs_get_entities( $option_manager->get_option( $module_type . 's', [] ), FS_Plugin::get_class_name() );
		}

		return [
			self::OPTION_NAME_PLUGINS => fs_get_entities( $option_manager->get_option( self::OPTION_NAME_PLUGINS, [] ), FS_Plugin::get_class_name() ),
			self::OPTION_NAME_THEMES  => fs_get_entities( $option_manager->get_option( self::OPTION_NAME_THEMES, [] ), FS_Plugin::get_class_name() ),
		];
	}
